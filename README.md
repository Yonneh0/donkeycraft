...
- **Speed indicator** — Top-right slider control with soft rounded background and outline frame; shows speed stop points with emoji icons (🐌 sneak, 🚶 walk, 🏃 run for survival; +🚀 turbo for creative grounded; +normal/fast/turbo/ultra for flying); click/drag to select speed
...
The project includes a CLI build system that formats, lints, and compiles all source files into self-contained HTML builds:

```bash
# Full pipeline (format → lint → compile)
node tools/build.js --all

# Compile only (format + lint + inline CSS/JS)
node tools/build.js --compile

# Format check only
node tools/build.js --format

# Lint check only
node tools/build.js --lint

# File state report (overview of all source files)
node tools/build.js --report
```

Fuzzy argument matching is supported — `--frm` resolves to `--format`.

### Build Outputs

| File | Description | Size |
|------|-------------|------|
| `build/donkeycraft-full.html` | Inlined, readable HTML with all CSS/JS embedded | ~2.89 MB |
| `build/donkeycraft-prod.html` | Minified production build | ~1.67 MB (53% compression) |

### Build Pipeline Stages

1. **Format** — Validates and auto-formats all JS/CSS files using Prettier (151 files: 126 JS + 25 CSS). Fails if any file is not properly formatted.
2. **Lint** — Runs ESLint on all JS files with strict mode (fails on warnings). Falls back to Node `--check` syntax validation if ESLint is unavailable.
3. **Compile** — Inlines all CSS and JS from `index.html`, generates full and production builds with detailed build report including file sizes and line counts.
...
```
file:///path/to/donkeycraft/build/donkeycraft-prod.html
```
...
README.md           # This file
package.json        # Node.js dependencies (dev: prettier, eslint)
tools/              # Build system
  build.js          # CLI entry point — pipeline orchestration (format → lint → compile)
  args.js           # Fuzzy argument parser with Levenshtein + Jaro-Winkler similarity
  format.js         # Prettier auto-formatting & validation
  lint.js           # ESLint + Stylelint integration with fallback syntax checking
  compile.js        # HTML compilation with CSS/JS inlining and minification
  project.json      # Build configuration (project name, input/output paths, tool settings)
  .prettierrc       # Prettier configuration (2-space indent, semicolons, trailing commas)
ref/                # Temporary development references (local)
build/              # Compiled output (generated by build system)
  donkeycraft-full.html   # Inlined, readable HTML build
  donkeycraft-prod.html   # Minified production HTML build
src/                # Source code
