...
(function() {
    ...
    Donkeycraft.RedstoneRepeaterComparator = (function() {
        ...
        function _processRepeater(entry, chunk) {
            ...
            var inputStrength = _getInputSignal(entry.x, entry.y, entry.z, state.facing);

            if (inputStrength > 0 && !state.active) {
                // Input detected: start delay timer
                state.active = true;
                state.targetTick = (Donkeycraft.RedstoneEngine ? Donkeycraft.RedstoneEngine.getCurrentTick() : 0) + state.delay;
            }
        ...
        function _getInputSignal(x, y, z, facing) {
            ...
            if (blockId === 173 || blockId === 229) {
                ...
                if (Donkeycraft._redstoneWiring && Donkeycraft._redstoneWiring.getSignalStrength) {
                    return Donkeycraft._redstoneWiring.getSignalStrength(inputPos.x, inputPos.y, inputPos.z);
                }
            }
        ...
        function _processComparator(entry, chunk) {
            ...
            var behindPos = _getInputPosition(entry.x, entry.y, entry.z, state.facing);
            if (!behindPos) return;
            ...
            var behindBlockId = behindChunk.getBlock(behindLocalX, behindPos.y, behindLocalZ);

            // Calculate input strengths
            state.inputStrengthA = _calculateContainerSignal(behindBlockId, behindPos.x, behindPos.y, behindPos.z, behindChunk);
            state.inputStrengthB = _getAdjacentWireSignal(entry.x, entry.y, entry.z, state.facing);

            // Calculate output based on mode
            var newOutput;
            if (state.mode === COMP_MODE_BLOCK_COMPARE) {
                // Block compare mode: output = inputA if inputA >= inputB
                newOutput = state.inputStrengthA >= state.inputStrengthB ? state.inputStrengthA : 0;
            } else {
        ...
        function _calculateContainerSignal(blockId, x, y, z, chunk) {
            ...
            if (blockId === 54 || blockId === 61) { // chest or furnace
            ...
            if (blockId === 173 || blockId === 229) {
                var chunkKey = Donkeycraft.WorldUtils.makeChunkKey(Math.floor(x / CHUNK_SIZE), Math.floor(z / CHUNK_SIZE));
                if (Donkeycraft._redstoneWiring && Donkeycraft._redstoneWiring.getSignalStrength) {
        ...
        function _getAdjacentWireSignal(x, y, z, facing) {
            ...
            if (sideBlockId === 173 || sideBlockId === 229) {
                if (Donkeycraft._redstoneWiring && Donkeycraft._redstoneWiring.getSignalStrength) {
                    return Donkeycraft._redstoneWiring.getSignalStrength(sidePos.x, sidePos.y, sidePos.z);
                }
            }
