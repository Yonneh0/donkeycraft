...
(function () {
  ...
  Donkeycraft.LevelData.prototype.getPlayerData = function () {
    ...
    return {
      position: {
        x: this._playerData.position.x,
        y: this._playerData.position.y,
        z: this._playerData.position.z,
      },
      rotation: {
        yaw: this._playerData.rotation.yaw,
        pitch: this._playerData.rotation.pitch,
      },
      ...
      inventory: this._playerData.inventory
        ? this._playerData.inventory.slice()
        : [],
      foodLevel:
        this._playerData.foodLevel !== undefined
          ? this._playerData.foodLevel
          : 12,
      hydration:
        this._playerData.hydration !== undefined
          ? this._playerData.hydration
          : 6.0,
      experience: {
        levels: this._playerData.experience.levels,
        points: this._playerData.experience.points,
      },
  ...
  Donkeycraft.LevelData.prototype.setPlayerHealth = function (health) {
    ...
    var clampedHealth;
    if (typeof Donkeycraft.clamp === 'function') {
      clampedHealth = Donkeycraft.clamp(
        Math.round(health || 0),
        0,
        this._playerData.maxHealth
      );
    } else {
      clampedHealth = Math.max(
        0,
        Math.min(Math.round(health || 0), this._playerData.maxHealth)
      );
    }
  ...
  Donkeycraft.LevelData.prototype.deserialize = function (data) {
    ...
    if (data.spawn && typeof data.spawn === 'object') {
      this._spawnX =
        typeof data.spawn.x === 'number'
          ? Math.round(data.spawn.x)
          : Donkeycraft.DEFAULT_SPAWN_X;
      this._spawnY =
        typeof data.spawn.y === 'number'
          ? Math.round(data.spawn.y)
          : Donkeycraft.DEFAULT_SPAWN_Y;
      this._spawnZ =
        typeof data.spawn.z === 'number'
          ? Math.round(data.spawn.z)
          : Donkeycraft.DEFAULT_SPAWN_Z;
    }

    if (typeof data.gameMode === 'string') {
      var validModes = ['survival', 'creative', 'spectator'];
      this._gameMode =
        validModes.indexOf(data.gameMode) !== -1 ? data.gameMode : 'survival';
    }
    ...
    if (data.playerData && typeof data.playerData === 'object') {
      this._playerData = {
        ...
        health:
          data.playerData.health !== undefined ? data.playerData.health : 20,
        ...
        foodLevel:
          data.playerData.foodLevel !== undefined
            ? data.playerData.foodLevel
            : 12,
        hydration:
          data.playerData.hydration !== undefined
            ? data.playerData.hydration
            : 6.0,
        experience: data.playerData.experience || { levels: 0, points: 0 },
        fallDistance: data.playerData.fallDistance || 0,
        alive:
          data.playerData.alive !== undefined ? data.playerData.alive : true,
      };
    } else {
      this._playerData = null;
    }
  ...
  Donkeycraft.LevelData.prototype.isValid = function () {
    ...
    var worldHeight =
      Donkeycraft.Config && Donkeycraft.Config.WORLD_HEIGHT !== undefined
   ...
   */
  Donkeycraft.LevelData.prototype.startAutoSave = function (
    worldStore,
    worldName,
    intervalMs
  ) {
    ...
    return this._worldStore
      ...
      .then(function (worldData) {
        ...
        return self._worldStore
          .saveWorld(self._worldNameRef, levelData, existingChunks)
          .then(function (saveSuccess) {
            if (!saveSuccess) {
              Donkeycraft.Logger.warn(
                'LevelData',
                'Failed to save world data for: ' + self._worldNameRef
              );
              return false;
            }
            ...
            if (self._worldStore.saveDirtyChunks) {
              return self._worldStore
                .saveDirtyChunks(self._worldNameRef)
                .then(function () {
                  return true;
                })
                .catch(function () {
                  Donkeycraft.Logger.warn(
                    'LevelData',
                    'World data saved but dirty chunks failed for: ' +
                      self._worldNameRef
                  );
