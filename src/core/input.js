...
(function () {
   ...
   *
   * Note: The 'F3' key is blocked by default to prevent browser dev tools
   * from opening during gameplay. Use a different key combination if you
   * need developer tools while the game is running.
   *
   * @constructor
   */
  Donkeycraft.Input = function () {
    ...
    /** @type {Object.<string, boolean>} — Raw just-pressed flags from key events (catches same-frame press+release). */
    this._keyJustPressedRaw = {};

    // --- Mouse state tracking ---

    /** @type {{left: boolean, right: boolean, middle: boolean, x: number, y: number, deltaX: number, deltaY: number}} */
    this._mouseState = {
      left: false,
      right: false,
      middle: false,
    ...
    /** @type {boolean} — Set asynchronously by the 'pointerlockchange' event. */
   ...
   * Exits pointer lock unconditionally if it is currently active,
   * regardless of the internal _mouseCaptured flag (which is set
  ...
  Donkeycraft.Input.prototype.updateMouseButtonStates = function () {
    ...
    Object.keys(this._mouseState).forEach(function (btn) {
      if (btn === 'x' || btn === 'y' || btn === 'deltaX' || btn === 'deltaY')
        return;
  ...
  /**
   * Key down event handler — updates _keyStates and _keyJustPressedRaw.
   * Ignores auto-repeat events (e.repeat) for just-pressed tracking.
   * @private
  ...
  /**
   * Key up event handler — clears _keyStates for the key.
   * Does NOT clear _keyJustPressedRaw; updateKeyStates() handles cleanup.
   * @private
   * @param {KeyboardEvent} e - The keyboard event.
   */
  ...
  /**
   * Mouse down event handler — updates button state and sets raw just-pressed flag.
   * @private
  ...
  Donkeycraft.Input.prototype._onMouseDown = function (e) {
    switch (e.button) {
      case 0:
        this._mouseState.left = true;
        this._mouseButtonJustPressedRaw.left = true;
        break;
      case 1:
        this._mouseState.middle = true;
        this._mouseButtonJustPressedRaw.middle = true;
        break;
      case 2:
        this._mouseState.right = true;
        this._mouseButtonJustPressedRaw.right = true;
        break;
    }
  };

  /**
   * Mouse up event handler — clears the corresponding button state.
   * @private
   * @param {MouseEvent} e - The mouse event.
   */
  Donkeycraft.Input.prototype._onMouseUp = function (e) {
    switch (e.button) {
      case 0:
        this._mouseState.left = false;
        break;
      case 1:
        this._mouseState.middle = false;
        break;
      case 2:
        this._mouseState.right = false;
        break;
    }
  };

  /**
   * Mouse move event handler — updates absolute position and accumulates delta when pointer locked.
   * @private
   * @param {MouseEvent} e - The mouse event with movementX/movementY.
   */
  ...
  /**
   * Wheel event handler — accumulates deltaY and prevents default scrolling.
   * @private
  ...
  /**
   * Context menu event handler — blocks the browser's right-click context menu.
   * @private
  ...
  /**
   * Pointer lock change event handler — updates _mouseCaptured based on document.pointerLockElement.
   * @private
   */
