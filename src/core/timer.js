...
(function () {
  ...
  Donkeycraft.Timer = function (ticksPerSecond) {
    ticksPerSecond =
      ticksPerSecond ||
      (Donkeycraft.Config &&
      typeof Donkeycraft.Config.GAME_TICKS_PER_SECOND === 'number'
        ? Donkeycraft.Config.GAME_TICKS_PER_SECOND
        : 20);
  ...
  Donkeycraft.Timer.prototype.start = function () {
    ...
    this._elapsed = 0;
    var self = this;
    this._animationFrameId = requestAnimationFrame(function (ts) {
      self._loop(ts);
    });
  };
