...
(function () {
    ...
    Donkeycraft.Matrix4.createPerspective = function (fovRadians, aspect, near, far) {
        ...
        if (!isFinite(fovRadians) || fovRadians <= 0 || fovRadians >= Math.PI) {
            Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
                ? Donkeycraft.Logger.warn('Matrix4', 'createPerspective: invalid fovRadians=' + fovRadians)
                : console.warn('Matrix4.createPerspective: invalid fovRadians=' + fovRadians);
            return Donkeycraft.Matrix4.createIdentity();
        }
        if (!isFinite(aspect) || aspect <= 0) {
            Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
                ? Donkeycraft.Logger.warn('Matrix4', 'createPerspective: invalid aspect=' + aspect)
                : console.warn('Matrix4.createPerspective: invalid aspect=' + aspect);
            return Donkeycraft.Matrix4.createIdentity();
        }
        if (!isFinite(near) || near <= 0) {
            Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
                ? Donkeycraft.Logger.warn('Matrix4', 'createPerspective: invalid near=' + near)
                : console.warn('Matrix4.createPerspective: invalid near=' + near);
            return Donkeycraft.Matrix4.createIdentity();
        }
        if (!isFinite(far) || far <= near) {
            Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
                ? Donkeycraft.Logger.warn('Matrix4', 'createPerspective: invalid far=' + far + ' (must be > near)')
                : console.warn('Matrix4.createPerspective: invalid far=' + far + ' (must be > near)');
            return Donkeycraft.Matrix4.createIdentity();
        }
    ...
    Donkeycraft.Matrix4.createOrthographic = function (left, right, bottom, top, near, far) {
        ...
        if (dx === 0 || dy === 0 || dz === 0) {
            Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
                ? Donkeycraft.Logger.warn('Matrix4', 'createOrthographic: zero-volume frustum')
    ...
    Donkeycraft.Matrix4.createLookAt = function (eye, target, up) {
        ...
        var dx = eye.x - target.x, dy = eye.y - target.y, dz = eye.z - target.z;
        if (dx === 0 && dy === 0 && dz === 0) {
            Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
                ? Donkeycraft.Logger.warn('Matrix4', 'createLookAt: eye and target are identical')
    ...
    Donkeycraft.Matrix4.prototype.transformVector = function (v, w) {
        ...
        if (!v || !(v instanceof Donkeycraft.Vector3)) {
            throw new TypeError('Matrix4.transformVector: first argument must be a Vector3');
        }
        var d = this._data;
    ...
    Donkeycraft.Quaternion.slerp = function (a, b, t) {
        ...
        if (!a || !(a instanceof Donkeycraft.Quaternion)) {
            throw new TypeError('Quaternion.slerp: first argument must be a Quaternion');
        }
        if (!b || !(b instanceof Donkeycraft.Quaternion)) {
            throw new TypeError('Quaternion.slerp: second argument must be a Quaternion');
        }
    ...
    Donkeycraft.map = function (value, inMin, inMax, outMin, outMax) {
        if (!isFinite(value) || !isFinite(inMin) || !isFinite(inMax)
            || !isFinite(outMin) || !isFinite(outMax)) {
            return NaN;
        }
