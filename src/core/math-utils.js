...
(function () {
  ...
  /**
   * Check if this vector is zero (or near-zero) using epsilon comparison.
   * @param {number} [epsilon=0.001] — Tolerance for approximate zero comparison.
   * @returns {boolean} True if all components are within epsilon of zero.
   */
  Donkeycraft.Vector3.prototype.isZero = function (epsilon) {
    epsilon = epsilon !== undefined ? epsilon : 0.001;
    return (
  ...
  /**
   * Matrix4 — 4×4 column-major matrix suitable for WebGL shaders.
   * Data is stored in a Float32Array of length 16, indexed by column-major layout:
   * element at row r, column c is at index `r + c * 4`.
   * @param {Float32Array} [data=null] — Underlying storage. A new array is allocated if omitted or null.
   * @throws {TypeError} If data is provided but is not a Float32Array of length 16.
   */
  Donkeycraft.Matrix4 = function (data) {
    if (data instanceof Float32Array && data.length === 16) {
      this._data = data;
    } else if (data && !(data instanceof Float32Array)) {
      throw new TypeError('Matrix4: data must be a Float32Array of length 16');
    } else {
   ...
   * Validates parameters and returns identity matrix on invalid input.
   * @param {number} fovRadians — Field of view in radians (must be positive and less than π).
   * @param {number} aspect — Aspect ratio (width/height, must be positive).
   * @param {number} near — Near clipping plane distance (must be positive).
   * @param {number} far — Far clipping plane distance (must be greater than near).
   * @returns {Donkeycraft.Matrix4} Perspective projection matrix. Returns identity on invalid input.
   * @throws {Error} Logs a warning and returns identity if parameters are invalid.
   */
  Donkeycraft.Matrix4.createPerspective = function (
    fovRadians,
    aspect,
    near,
    far
    ...
    // Validate parameters
    if (!isFinite(fovRadians) || fovRadians <= 0 || fovRadians >= Math.PI) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn(
            'Matrix4',
            'createPerspective: invalid fovRadians=' + fovRadians
          )
        : console.warn(
            'Matrix4.createPerspective: invalid fovRadians=' + fovRadians
          );
    ...
    if (!isFinite(aspect) || aspect <= 0) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn(
            'Matrix4',
            'createPerspective: invalid aspect=' + aspect
          )
        : console.warn('Matrix4.createPerspective: invalid aspect=' + aspect);
      return Donkeycraft.Matrix4.createIdentity();
    }
    if (!isFinite(near) || near <= 0) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn(
            'Matrix4',
            'createPerspective: invalid near=' + near
          )
        : console.warn('Matrix4.createPerspective: invalid near=' + near);
      return Donkeycraft.Matrix4.createIdentity();
    }
    if (!isFinite(far) || far <= near) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn(
            'Matrix4',
            'createPerspective: invalid far=' + far + ' (must be > near)'
          )
        : console.warn(
            'Matrix4.createPerspective: invalid far=' +
              far +
              ' (must be > near)'
          );
      return Donkeycraft.Matrix4.createIdentity();
   ...
   * Validates parameters and returns identity matrix on invalid input.
   * @param {number} left — Left clipping plane coordinate.
   * @param {number} right — Right clipping plane coordinate (must differ from left).
   * @param {number} bottom — Bottom clipping plane coordinate.
   * @param {number} top — Top clipping plane coordinate (must differ from bottom).
   * @param {number} near — Near clipping plane distance (must differ from far).
   * @param {number} far — Far clipping plane distance (must differ from near).
   * @returns {Donkeycraft.Matrix4} Orthographic projection matrix. Returns identity on invalid input.
   */
  Donkeycraft.Matrix4.createOrthographic = function (
    left,
    right,
    bottom,
    top,
    ...
    if (dx === 0 || dy === 0 || dz === 0) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn(
            'Matrix4',
            'createOrthographic: zero-volume frustum'
          )
        : console.warn('Matrix4.createOrthographic: zero-volume frustum');
      return Donkeycraft.Matrix4.createIdentity();
    }
    var data = new Float32Array(16);
    data[0] = 2 / dx;
   ...
   * Validates inputs and returns identity matrix if eye and target are identical
   * (no viewing direction) or if up vector is zero.
   * @param {Donkeycraft.Vector3} eye — Camera position.
   * @param {Donkeycraft.Vector3} target — Point to look at.
   * @param {Donkeycraft.Vector3} up — Up direction (typically [0,1,0]).
   * @returns {Donkeycraft.Matrix4} View matrix. Returns identity if inputs are degenerate.
   */
  Donkeycraft.Matrix4.createLookAt = function (eye, target, up) {
    ...
    if (dx === 0 && dy === 0 && dz === 0) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn(
            'Matrix4',
            'createLookAt: eye and target are identical'
          )
        : console.warn('Matrix4.createLookAt: eye and target are identical');
      return Donkeycraft.Matrix4.createIdentity();
    }
    // Validate up vector is non-zero
    if (up.x === 0 && up.y === 0 && up.z === 0) {
      Donkeycraft.Logger && typeof Donkeycraft.Logger.warn === 'function'
        ? Donkeycraft.Logger.warn('Matrix4', 'createLookAt: up vector is zero')
        : console.warn('Matrix4.createLookAt: up vector is zero');
      return Donkeycraft.Matrix4.createIdentity();
    }
  ...
  Donkeycraft.Matrix4.createRotation = function (angle, axis) {
    ...
    if (len > 0.00001) {
      ...
      x = 0;
      y = 1;
      z = 0; // Default to Y-axis if axis is zero
    }
  ...
  /**
   * Transform a vector by this matrix (column-major 4×4).
   * When w=0 (default), the vector is treated as a direction — translation is ignored.
   * When w=1, the vector is treated as a position — translation is applied.
   * @param {Donkeycraft.Vector3} v — The vector to transform.
   * @param {number} [w=0] — W component: 0 for direction vectors, 1 for position vectors.
   *    Pass 0 explicitly to treat as a direction (the default `w || 0` would incorrectly
   *    treat w=0 as undefined).
   * @returns {Donkeycraft.Vector3} The transformed vector.
   * @throws {TypeError} If v is not a Vector3 instance.
   */
  Donkeycraft.Matrix4.prototype.transformVector = function (v, w) {
    // Validate input
    if (!v || !(v instanceof Donkeycraft.Vector3)) {
      throw new TypeError(
        'Matrix4.transformVector: first argument must be a Vector3'
      );
    }
    var d = this._data;
    var x = v.x,
      y = v.y,
      z = v.z;
    // Explicit undefined check: w=0 should be used (direction), not treated as falsy
    var ww = w !== undefined ? w : 0;
   ...
   * @returns {Donkeycraft.Quaternion} A quaternion representing no rotation.
   */
   ...
   * Uses SLERP for smooth rotation interpolation with shortest-path handling.
   * When quaternions are nearly identical, falls back to LERP for numerical stability
   * (the SLERP formula would involve division by a near-zero sinAngle).
   * @param {Donkeycraft.Quaternion} a - Start quaternion (must be normalized).
   * @param {Donkeycraft.Quaternion} b - End quaternion (must be normalized).
   * @param {number} t - Interpolation factor [0, 1]. Clamped internally.
   * @returns {Donkeycraft.Quaternion} A new normalized quaternion.
   * @throws {Error} If either input is not a valid Quaternion instance.
   */
  Donkeycraft.Quaternion.slerp = function (a, b, t) {
    // Validate inputs
    if (!a || !(a instanceof Donkeycraft.Quaternion)) {
      throw new TypeError(
        'Quaternion.slerp: first argument must be a Quaternion'
      );
    }
    if (!b || !(b instanceof Donkeycraft.Quaternion)) {
      throw new TypeError(
        'Quaternion.slerp: second argument must be a Quaternion'
      );
    }
    ...
    // Clamp t to [0, 1] for valid interpolation range
    t = Math.min(1, Math.max(0, t));

    var cosTheta = ax * bx + ay * by + az * bz + aw * bw;

    // If cosTheta < 0, the quaternions are on opposite sides of the hypersphere —
    // flip one for shortest-path interpolation (avoids rotating the long way around)
    var flip = 1;
    if (cosTheta < 0) {
      flip = -1;
      cosTheta = -cosTheta;
    ...
    if (cosClamped > 0.9999) {
      // Quaternions are very close — SLERP formula degenerates (sinAngle ≈ 0).
  ...
  Donkeycraft.Quaternion.prototype.toEuler = function () {
    ...
    var pitch = Math.asin(
      Math.min(1, Math.max(-1, 2 * (q.w * q.y - q.z * q.x)))
    );
    var roll = Math.atan2(
      2 * (q.w * q.x + q.y * q.z),
      1 - 2 * (q.x * q.x + q.y * q.y)
  ...
  /**
   * PerlinNoise — Canonical Perlin noise implementation (1D, 2D, 3D + FBM).
   * Uses a Numerical Recipes LCG (multiplier 16807, modulus 2147483647) for permutation table generation.
   * All terrain and feature generators delegate to this module to ensure spatial coherence.
   * @namespace
   */
  Donkeycraft.PerlinNoise = (function () {
     ...
     * Uses Numerical Recipes LCG (multiplier 16807, modulus 2¹³¹−1) for deterministic shuffling.
     * @param {number} [seed=42] — Seed value. Clamped to integer; defaults to 42 if null/undefined/NaN.
     */
    function init(seed) {
      _initialized = true;
      ...
      // Clamp seed to valid integer range [1, 2147483646]
      var s =
        seed !== undefined &&
        seed !== null &&
        typeof seed === 'number' &&
    ...
    /**
     * 3D Perlin noise with input validation.
     * @param {number} x
     * @param {number} y
     * @param {number} z
     * @returns {number}
    ...
    /**
     * 2D Perlin noise (delegates to 3D with z=0).
     * @param {number} x
     * @param {number} y
     * @returns {number}
     */
    ...
    /**
     * 1D Perlin noise (delegates to 3D with y=z=0).
     * @param {number} x
     * @returns {number}
     */
    function noise1D(x) {
    ...
    // Wrap internal noise functions with validation for public API.
    function makeValidatedNoise3D(fn) {
      return function (x, y, z) {
        if (!isFinite(x) || !isFinite(y) || !isFinite(z)) return 0;
        return fn(x, y, z);
      };
    }
    function makeValidatedNoise2D(fn) {
      return function (x, y) {
        if (!isFinite(x) || !isFinite(y)) return 0;
        return fn(x, y);
      };
    }
    function makeValidatedNoise1D(fn) {
      return function (x) {
        if (!isFinite(x)) return 0;
        return fn(x);
      };
    ...
    return {
      ...
      noise2D: makeValidatedNoise2D(noise2D),
   ...
   * @param {number} t - Interpolation factor. Values outside [0,1] produce extrapolation.
   * @returns {number} The interpolated value: `a + (b - a) * t`.
   */
   ...
   * @returns {number} The clamped value, or NaN if inputs are invalid.
   ...
   * When inMin equals inMax, returns outMin (avoids division by zero).
   * @param {number} value — Value to remap.
   * @param {number} inMin — Input range minimum.
   * @param {number} inMax — Input range maximum.
   * @param {number} outMin — Output range minimum.
   * @param {number} outMax — Output range maximum.
   * @returns {number} The remapped value, or outMin if inMin === inMax.
   */
   ...
   * @returns {number} The rounded integer, or 0 if input is not finite.
   */
  ...
  /**
   * Check if a value falls within a range (inclusive of both endpoints).
   * @param {number} value — Value to check.
   * @param {number} min — Range minimum.
   * @param {number} max — Range maximum.
   * @returns {boolean} True if min ≤ value ≤ max, false otherwise.
   */
