...
(function () {
   ...
   */
  Donkeycraft.MeshOptimizer.prototype._validateGeometry = function (
    geometry,
    name
  ) {
    if (!geometry || typeof geometry !== 'object') {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        name +
          ': geometry is null, undefined, or not an object — returning original geometry'
      );
      return false;
    }
    if (!(geometry.vertices instanceof Float32Array)) {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        name + ': vertices is not a Float32Array — returning original geometry'
      );
      return false;
    }
    if (
      typeof geometry.vertexCount !== 'number' ||
      typeof geometry.indexCount !== 'number'
    ) {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        name +
          ': vertexCount or indexCount missing or invalid — returning original geometry'
      );
      return false;
    }
    return true;
   ...
   * @returns {boolean} True if valid.
   */
  Donkeycraft.MeshOptimizer.prototype._validateCameraPos = function (
    cameraPos
  ) {
   ...
   */
  Donkeycraft.MeshOptimizer.prototype.generateIndexBuffer = function (
    geometry,
    epsilon
  ) {
    // Validate inputs
    ...
    if (epsilon !== undefined && (isNaN(epsilon) || epsilon < 0)) {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        'generateIndexBuffer: invalid epsilon (' +
          epsilon +
          ') — using default 0.001'
      );
      epsilon = 0.001;
    ...
    if (vertexCount > maxVertices) {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        'generateIndexBuffer: vertexCount (' +
          vertexCount +
    ...
    if (geometry.indices && geometry.indexCount > 0) {
      ...
      for (var i = 0; i < geometry.indexCount; i++) {
        ...
        if (origIdx >= 0 && origIdx < vertexCount) {
          ...
          Donkeycraft.Logger.warn(
            'MeshOptimizer',
            'generateIndexBuffer: index ' +
              i +
              ' references out-of-bounds vertex ' +
              origIdx +
              ' — using 0'
          );
          indices[i] = 0;
   ...
   */
  Donkeycraft.MeshOptimizer.prototype.cullBackFaces = function (
    geometry,
    cameraPos
  ) {
    // Validate inputs
    ...
    if (!this._validateCameraPos(cameraPos)) {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        'cullBackFaces: invalid camera position — returning original geometry'
      );
      return geometry;
    }
    ...
    for (var i = 0; i < indexCount; i += 3) {
      ...
      if (i0 >= vertexCount || i1 >= vertexCount || i2 >= vertexCount) {
        Donkeycraft.Logger.warn(
          'MeshOptimizer',
          'cullBackFaces: index references out-of-bounds vertex — skipping triangle at index ' +
            i
        );
        continue;
      }
      ...
      var v0x = vertices[base0],
        v0y = vertices[base0 + 1],
        v0z = vertices[base0 + 2];
      var v1x = vertices[base1],
        v1y = vertices[base1 + 1],
        v1z = vertices[base1 + 2];
      var v2x = vertices[base2],
        v2y = vertices[base2 + 1],
        v2z = vertices[base2 + 2];
   ...
   */
  Donkeycraft.MeshOptimizer.prototype.optimize = function (
    geometry,
    cameraPos,
    cullBackFaces,
    epsilon
    ...
    if (!geometry || typeof geometry !== 'object') {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        'optimize: geometry is null or undefined — returning original'
      );
      return geometry;
    }
    ...
    if (result.vertexCount === 0 || result.indexCount === 0) {
      Donkeycraft.Logger.warn(
        'MeshOptimizer',
        'optimize: resulting geometry is empty (verts: ' +
          result.vertexCount +
          ', indices: ' +
          result.indexCount +
          ') — returning original (' +
