...
(function () {
  ...
  Donkeycraft.Weather.prototype.setWeather = function (state) {
    ...
    if (
      state === Donkeycraft.WEATHER_THUNDER &&
      this._weatherState !== Donkeycraft.WEATHER_RAIN
    ) {
   ...
   * @param {Object} [biomeAtPlayer] — Optional biome info for restrictions.
   */
  Donkeycraft.Weather.prototype.changeToRandomWeather = function (
    biomeAtPlayer
  ) {
    var possibleStates = [
      Donkeycraft.WEATHER_CLEAR,
   ...
   */
  Donkeycraft.Weather.prototype.setBiomeWeatherRestriction = function (
    biomeId,
    permissions
  ) {
   ...
   * @returns {{canRain: boolean, canSnow: boolean}} Weather permissions.
   */
  Donkeycraft.Weather.prototype.getBiomeWeatherRestrictions = function (
    biomeId
  ) {
   ...
   * @private
   */
  Donkeycraft.WeatherRenderer.prototype._spawnParticle = function (
    x,
    y,
    z,
    type
   ...
   * @returns {void}
   */
  Donkeycraft.WeatherRenderer.prototype.update = function (
    deltaTime,
    playerPos
  ) {
   ...
   */
  Donkeycraft.WeatherRenderer.prototype.render = function (
    camera,
    particleDensity,
    type
  ) {
    ...
    try {
      ...
      var totalFloats =
        this._particleCount * vertsPerParticle * floatsPerVertex;
      ...
      for (var i = 0; i < this._particleCount; i++) {
        ...
        var rx = right.x * particleSize,
          ry = right.y * particleSize,
          rz = right.z * particleSize;
        var ux = up.x * particleSize,
          uy = up.y * particleSize,
          uz = up.z * particleSize;
        ...
        vertices[base + 6] = g;
        vertices[base + 7] = b;
        vertices[base + 8] = alpha;
        // Bottom-right
        vertices[base + 9] = brx;
        vertices[base + 10] = bry;
        vertices[base + 11] = brz;
        vertices[base + 12] = 1;
        vertices[base + 13] = 0;
        vertices[base + 14] = r;
        vertices[base + 15] = g;
        vertices[base + 16] = b;
        vertices[base + 17] = alpha;
        // Top-right
        vertices[base + 18] = trx;
        vertices[base + 19] = topRightY;
        vertices[base + 20] = trz;
        vertices[base + 21] = 1;
        vertices[base + 22] = 1;
        vertices[base + 23] = r;
        vertices[base + 24] = g;
        vertices[base + 25] = b;
        vertices[base + 26] = alpha;
        // Bottom-left (duplicate for second triangle)
        vertices[base + 27] = blx;
        vertices[base + 28] = bly;
        vertices[base + 29] = blz;
        vertices[base + 30] = 0;
        vertices[base + 31] = 0;
        vertices[base + 32] = r;
        vertices[base + 33] = g;
        vertices[base + 34] = b;
        vertices[base + 35] = alpha;
        // Top-right (duplicate)
        vertices[base + 36] = trx;
        vertices[base + 37] = topRightY;
        vertices[base + 38] = trz;
        vertices[base + 39] = 1;
        vertices[base + 40] = 1;
        vertices[base + 41] = r;
        vertices[base + 42] = g;
        vertices[base + 43] = b;
        vertices[base + 44] = alpha;
        // Top-left
        vertices[base + 45] = tlx;
        vertices[base + 46] = tly;
        vertices[base + 47] = tlz;
        vertices[base + 48] = 0;
        vertices[base + 49] = 1;
        vertices[base + 50] = r;
        vertices[base + 51] = g;
        vertices[base + 52] = b;
        vertices[base + 53] = alpha;
      }
      ...
      gl.bufferData(
        gl.ARRAY_BUFFER,
        vertices.subarray(0, totalFloats),
        gl.DYNAMIC_DRAW
      );
   ...
   * @returns {void}
   */
  Donkeycraft.WeatherRenderer.prototype.spawnInitialParticles = function (
    count
  ) {
