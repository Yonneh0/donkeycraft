...
**Vertex data generation** for chunk meshes. Builds position, UV, normal, and light arrays with face culling, Y-bounds optimization, and three-tier UV lookup.
...
|----------|-------|-------------|
| `ATLAS_GRID` | `16` | Number of texture tiles per row/column in the atlas grid |
| `MAX_BLOCK_ID` | `256` | Maximum block ID that fits in the atlas (IDs ≥ 256 fall back to placeholder) |

### Methods

| Method | Returns | Description |
|--------|---------|-------------|
| `buildChunk(chunkX, chunkZ, getBlockFunc)` | `{vertices, indices, vertexCount, indexCount, useUint32}` | Build full chunk geometry with face culling and Y-bounds optimization |
| `buildQuad(size, y)` | `{vertices, indices, vertexCount, indexCount}` | Build a simple test quad (4 verts, flat upward-facing) |
| `buildCube(size, y)` | `{vertices, indices, vertexCount, indexCount}` | Build a simple test cube (24 verts, 6 faces with per-face lighting) |
| `isTransparent(blockId)` | `boolean` | Check if block is transparent (delegates to BlockTypes, falls back to hardcoded IDs) |

### Private Methods

| Method | Returns | Description |
|--------|---------|-------------|
| `_computeChunkYBounds(getBlockFunc)` | `{minY, maxY}` | Compute Y-bounds for optimized mesh building (early exit scan) |
| `_getBlockAt(nx, ny, nz, getBlockFunc, chunkX, chunkZ)` | `number` | Get block at world coordinates with chunk boundary handling |
| `_getBlockUV(blockId, faceName)` | `{u0, v0, u1, v1}` | Three-tier UV lookup: BlockModelRegistry → TextureAtlas → tile-based fallback |
...
[4-5]   UV:        normalized texture atlas coordinates [0,1]
[6-8]   normal:    face normal vector (nx, ny, nz)
```

### Index Layout

6 indices per quad (two CCW triangles): `(v0, v1, v2)` and `(v0, v2, v3)`.

### Face Definitions

Six directional faces with baked light intensities following Minecraft-style lighting:

| Face | Normal | Light | Description |
|------|--------|-------|-------------|
| `+Y` (up) | `[0, 1, 0]` | 0.9 | Sun-facing, brightest |
| `-Y` (down) | `[0, -1, 0]` | 0.5 | Darkest, no direct light |
| `+Z` (south) | `[0, 0, 1]` | 0.9 | Direct sunlight exposure |
| `-Z` (north) | `[0, 0, -1]` | 0.6 | Least sun exposure |
| `+X` (east) | `[1, 0, 0]` | 0.8 | Medium-bright |
| `-X` (west) | `[-1, 0, 0]` | 0.8 | Medium |

### UV Mapping Strategy

Three-tier lookup ensures correct textures for all block types:

1. **Primary**: `BlockModelRegistry.getFaceUV(blockId, faceName)` — face-specific textures for special blocks (grass_block_top on top, grass_block_side on sides, log bark vs wood, chest front vs side, furnace front vs smooth stone)
2. **Secondary**: `TextureAtlas.getBlockUV(blockId)` static method — simple single-texture blocks
3. **Fallback**: Tile-based calculation using atlas grid layout (`col = blockId % 16`, `row = floor(blockId / 16)`)

Block IDs ≥ 256 fall back to placeholder texture (ID 0).

### Y-Bounds Optimization

`_computeChunkYBounds()` scans upward from Y=0 for minY and downward from world height for maxY, returning early once both bounds are found. This avoids iterating all 256 levels when terrain is sparse. Adds 1 block padding on each side for neighbor face culling at chunk edges.

### Error Handling

- Invalid chunk coordinates are clamped with a warning log
- Invalid `getBlockFunc` returns empty geometry with error log
- Invalid block IDs in block getter are skipped (treated as air)
- Out-of-bounds neighbor lookups return air (0) to expose chunk-edge faces
- UV lookup validates both blockId and faceName, logging warnings on invalid input
- `buildQuad`/`buildCube` enforce minimum size of 0.001 to prevent degenerate geometry
