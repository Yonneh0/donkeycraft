...
(function () {
    ...
    if (color === undefined) {
      ...
      if (_blockColors._unknownLog < 10) {
        Donkeycraft.Logger.warn(
          'MapRenderer',
          'Unknown block ID: ' + blockId + ' — using fallback color #555555'
        );
  ...
  Donkeycraft.MapRenderer.prototype.init = function () {
    try {
      ...
      this._minimapUI = new Donkeycraft.MinimapUI({
        chunkManager: this._chunkManager,
      });
      this._mapPanelUI = new Donkeycraft.MapPanelUI({
        chunkManager: this._chunkManager,
      });
      ...
      if (
        this._mapPanelUI &&
        typeof this._mapPanelUI.registerEscapeHandler === 'function'
      ) {
   ...
   */
  Donkeycraft.MapRenderer.prototype.setCanvases = function (
    fullMapCanvas,
    minimapCanvas
  ) {
    if (this._mapPanelUI)
      this._mapPanelUI.setCanvases(fullMapCanvas, minimapCanvas);
   ...
   */
  Donkeycraft.MapRenderer.prototype.renderFullMap = function (
    playerPos,
    yaw,
    pitch
  ) {
   ...
   */
  Donkeycraft.MapRenderer.prototype.renderMinimap = function (
    playerPos,
    yaw,
    pitch
  ) {
   ...
   */
  Donkeycraft.MapRenderer.invalidateChunkSurfaceMap = function (
    chunkX,
    chunkZ
  ) {
    // Invalidate MinimapUI surface map for this chunk
    if (
      Donkeycraft.MinimapUI &&
      typeof Donkeycraft.MinimapUI.invalidateChunkSurfaceMap === 'function'
    ) {
    ...
    if (
      Donkeycraft.MapPanelUI &&
      typeof Donkeycraft.MapPanelUI.invalidateChunkSurfaceMap === 'function'
    ) {
