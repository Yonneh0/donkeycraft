...
(function () {
  ...
  Donkeycraft.GUIRenderer.prototype._initBuffers = function () {
    ...
    if (this._crosshairGeometry) {
      ...
      gl.bufferData(
        gl.ARRAY_BUFFER,
        this._crosshairGeometry.vertices,
        gl.STATIC_DRAW
      );
      ...
      gl.bufferData(
        ...
        this._crosshairGeometry.indices,
        gl.STATIC_DRAW
      );
    }
   ...
   * @private
   */
  Donkeycraft.GUIRenderer.prototype._getCachedProjMatrix = function (
    canvasWidth,
    canvasHeight
  ) {
   ...
   */
  Donkeycraft.GUIRenderer.prototype.renderCrosshair = function (
    canvasWidth,
    canvasHeight
  ) {
    var gl = this._gl;
    ...
    try {
      ...
      gl.drawElements(
        gl.TRIANGLES,
        this._crosshairGeometry.indexCount,
        gl.UNSIGNED_SHORT,
        0
      );
   ...
   * @param {number} canvasHeight - Canvas height in pixels.
   */
  Donkeycraft.GUIRenderer.prototype.renderHotbar = function (
    selectedSlot,
    canvasWidth,
    canvasHeight
  ) {
    ...
    try {
      ...
      if (
        !this._hotbarColorBuffer ||
        this._hotbarColorBuffer.length !== this._hotbarGeometry.vertices.length
      ) {
        this._hotbarColorBuffer = new Float32Array(
          this._hotbarGeometry.vertices.length
        );
      }
      ...
      gl.drawElements(
        gl.TRIANGLES,
        this._hotbarGeometry.indexCount,
        gl.UNSIGNED_SHORT,
        0
      );
   ...
   * @param {number} canvasHeight - Canvas height in pixels.
   */
  Donkeycraft.GUIRenderer.prototype.renderHearts = function (
    health,
    maxHealth,
    canvasWidth,
    canvasHeight
   ...
   * @private
   */
  Donkeycraft.GUIRenderer.prototype._drawHeart = function (
    gl,
    x,
    y,
    size,
   ...
   * @param {number} canvasHeight - Canvas height in pixels.
   */
  Donkeycraft.GUIRenderer.prototype.renderHunger = function (
    food,
    maxFood,
    canvasWidth,
    canvasHeight
    ...
    try {
      ...
      for (var i = 0; i < foodCount; i++) {
        ...
        var drumstickColor;
        if (isFull) {
          drumstickColor = [0.65, 0.4, 0.2, 1.0]; // Brown
        } else if (isHalf) {
          drumstickColor = [0.65, 0.4, 0.2, 0.6]; // Semi-transparent brown
        } else {
   ...
   * @private
   */
  Donkeycraft.GUIRenderer.prototype._drawDrumstick = function (
    gl,
    x,
    y,
    size,
   ...
   * @param {number} canvasHeight - Canvas height in pixels.
   */
  Donkeycraft.GUIRenderer.prototype.renderDebugInfo = function (
    debugInfo,
    canvasWidth,
    canvasHeight
  ) {
   ...
   * @param {number} canvasHeight - Canvas height in pixels.
   */
  Donkeycraft.GUIRenderer.prototype.renderAll = function (
    state,
    canvasWidth,
    canvasHeight
  ) {
    ...
    try {
      ...
      if (state.hotbar !== false) {
        ...
        this.renderHotbar(
          slot !== undefined && slot >= 0 && slot <= 8 ? slot | 0 : 0,
          canvasWidth,
          canvasHeight
        );
      }
