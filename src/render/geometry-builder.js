...
(function () {
  ...
  Donkeycraft.GeometryBuilder.prototype.isWaterBlock = function (blockId) {
    ...
    if (
      Donkeycraft.BlockTypes &&
      typeof Donkeycraft.BlockTypes.isLiquid === 'function'
    ) {
    ...
    if (
      ...
      typeof Donkeycraft.WaterGenerator.isLiquidBlock === 'function'
    ) {
      if (Donkeycraft.WaterGenerator.isLiquidBlock(blockId)) {
        ...
        for (var i = 0; i < blocks.length; i++) {
          if (blocks[i].id === blockId && blocks[i].name === 'water')
            return true;
        }
      }
    }
   ...
   */
  Donkeycraft.GeometryBuilder.prototype._computeChunkYBounds = function (
    getBlockFunc
  ) {
    var minY = -1,
      maxY = -1;
   ...
   */
  Donkeycraft.GeometryBuilder.prototype.buildChunk = function (
    chunkX,
    chunkZ,
    getBlockFunc
  ) {
    ...
    if (!Number.isInteger(chunkX) || !Number.isInteger(chunkZ)) {
      Donkeycraft.Logger.warn(
        'GeometryBuilder',
        'buildChunk: non-integer chunk coordinates [' +
          chunkX +
          ',' +
          chunkZ +
          '] — clamping'
    ...
    if (typeof getBlockFunc !== 'function') {
      Donkeycraft.Logger.error(
        'GeometryBuilder',
        'buildChunk: getBlockFunc is not a function for chunk [' +
          chunkX +
          ',' +
          chunkZ +
          ']'
   ...
   */
  Donkeycraft.GeometryBuilder.prototype._getBlockAt = function (
    nx,
    ny,
    nz,
    getBlockFunc,
   ...
   */
  Donkeycraft.GeometryBuilder.prototype._getBlockUV = function (
    blockId,
    faceName
  ) {
    ...
    if (typeof faceName !== 'string' || faceName.length === 0) {
      Donkeycraft.Logger.warn(
        'GeometryBuilder',
        '_getBlockUV: invalid faceName "' +
          faceName +
          '" — falling back to tile-based UV'
      );
    ...
    if (
      Donkeycraft.BlockModelRegistry &&
      typeof Donkeycraft.BlockModelRegistry.getFaceUV === 'function'
    ) {
