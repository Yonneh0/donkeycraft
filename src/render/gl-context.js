...
(function () {
  ...
  Donkeycraft.GLContext = function (canvas) {
    ...
    if (!success) {
      Donkeycraft.Logger.error(
        'GLContext',
        'WebGL context creation failed during construction'
      );
    }
  };
  ...
  Donkeycraft.GLContext.prototype._createContext = function () {
    ...
    this._onContextLost = function (event) {
      ...
      Donkeycraft.Logger.error(
        'GLContext',
        'WebGL context lost — browser-initiated (GPU reset, tab crash, driver update)'
      );
    };
    ...
    try {
      ...
      if (!this._context) {
        Donkeycraft.Logger.error(
          'GLContext',
          'Failed to create WebGL 1 context'
        );
        return false;
      }
      ...
      Donkeycraft.Logger.error(
        'GLContext',
        'WebGL context creation threw error: ' + e.message
      );
      return false;
    }
  };
    ...
    if (this._context && !this._contextLost) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'GLContext',
          'loseContext not available: ' + e.message
        );
      }
    }
    this._context = null;
