...
(function () {
  ...
  Donkeycraft.WorldTime = function (totalTicks) {
    this._totalTicks =
      totalTicks && typeof totalTicks === 'number' && totalTicks > 0
        ? totalTicks
        : 0;
    /**
  ...
  Donkeycraft.WorldTime.prototype.getHour = function () {
    return Math.floor(
      this.getTicksInDay() / Donkeycraft.WORLD_TIME_TICKS_PER_HOUR
    );
  };
  ...
  Donkeycraft.WorldTime.prototype.getMinute = function () {
    var ticksInHour =
      this.getTicksInDay() % Donkeycraft.WORLD_TIME_TICKS_PER_HOUR;
    return Math.floor(
      ticksInHour / (Donkeycraft.WORLD_TIME_TICKS_PER_HOUR / 60)
    );
  };
  ...
  Donkeycraft.WorldTime.prototype.getMoonPhase = function () {
    return Math.floor(
      (this._totalTicks / (Donkeycraft.WORLD_TIME_MOON_PHASE_CYCLE / 8)) % 8
    );
  };
  ...
  Donkeycraft.WorldTime.todToTicks = function (tod) {
    return Math.floor(
      (((tod % 1) + 1) % 1) * Donkeycraft.WORLD_TIME_TICKS_PER_DAY
    );
  };
  ...
  Donkeycraft.WorldTime.ticksToTod = function (ticksInDay) {
    return (
      (ticksInDay % Donkeycraft.WORLD_TIME_TICKS_PER_DAY) /
      Donkeycraft.WORLD_TIME_TICKS_PER_DAY
    );
  };
  ...
  Donkeycraft.WorldTime.fromTimeOfDay = function (tod, currentTotal) {
    ...
    var days = currentTotal
      ? Math.floor(currentTotal / Donkeycraft.WORLD_TIME_TICKS_PER_DAY)
      : 0;
    return new Donkeycraft.WorldTime(
      days * Donkeycraft.WORLD_TIME_TICKS_PER_DAY + ticksInDay
    );
  };
