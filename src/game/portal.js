...
(function () {
  ...
  Donkeycraft.Portal = (function () {
    ...
    function _resolvePortalBlockIds() {
      ...
      var cryingObsidian =
        Donkeycraft.BlockRegistry.getBlockByName('crying_obsidian');
      if (cryingObsidian) _cryingObsidianId = cryingObsidian.id;

      var netherPortal =
        Donkeycraft.BlockRegistry.getBlockByName('nether_portal');
    ...
    function init(bus, chunkManager, dimensionType) {
      ...
      if (!chunkManager) {
        Donkeycraft.Logger.warn(
          'Portal',
          'ChunkManager not provided — block queries will return 0'
        );
      }
    }
    ...
    function switchDimension(dimensionType) {
      ...
      _chunkManager =
        Donkeycraft.Dimensions.getChunkManagerForDimension(dimensionType);
      _resolvePortalBlockIds();

      // Notify map renderer of dimension change (clears surface maps, updates label)
      if (
        Donkeycraft.MapRenderer &&
        Donkeycraft.MapRenderer.prototype.onDimensionChange
      ) {
        var dimName = Donkeycraft.Dimensions.getCurrentDimensionName
          ? Donkeycraft.Dimensions.getCurrentDimensionName()
          : 'Unknown';
        try {
          var gameRef = window._dkGame;
          if (
            gameRef &&
            gameRef._mapRenderer &&
            typeof gameRef._mapRenderer.onDimensionChange === 'function'
          ) {
      ...
      Donkeycraft.Logger.info(
        'Portal',
        'Switched to dimension: ' + dimensionType
      );
    }
    ...
    function detectNetherPortal(globalX, globalY, globalZ) {
      ...
      for (var d = 0; d < directions.length; d++) {
        ...
        var frameWidth = _countFrameLength(
          globalX,
          globalZ,
          dir.dx,
          dir.dz,
    ...
    function _verifyFrame(startX, startZ, dx, dz, startY) {
      ...
      var leftValid = _checkVerticalEdge(
        startX,
        startY,
        startZ,
        dx,
      ...
      var rightValid = _checkVerticalEdge(
        startX,
        startY,
        startZ,
        dx,
    ...
    function _checkTopBar(startX, startZ, startY, dx, dz, width) {
      for (var i = 0; i < width; i++) {
        if (
          !isPortalBlock(getBlockAt(startX + i * dx, startY, startZ + i * dz))
        ) {
          return false;
        }
      }
    ...
    function createPortal(worldX, worldY, worldZ, direction, dimensionType) {
      if (!_chunkManager) {
        Donkeycraft.Logger.error(
          'Portal',
          'Cannot create portal — ChunkManager is null'
        );
        return false;
      }
      ...
      dimensionType =
        dimensionType !== undefined ? dimensionType : _currentDimension;
      ...
      if (direction === 0) {
        dx = 1;
      } // North → East-West frame
      else if (direction === 2) {
        dx = -1;
      } // South → East-West frame
      else if (direction === 1) {
        dz = 1;
      } // East → North-South frame
      else if (direction === 3) {
        dz = -1;
      } // West → North-South frame
      ...
      var dimKey =
        typeof dimensionType === 'number'
          ? dimensionType === Donkeycraft.DimensionType.NETHER
            ? 'nether'
            : dimensionType === Donkeycraft.DimensionType.END
              ? 'end'
              : 'overworld'
          : 'overworld';
      var key =
    ...
    function findMatchingPortal(x, y, z, dimensionType) {
      ...
      if (
        dimensionType !== _currentDimension ||
        savedChunkManager !== _chunkManager
      ) {
    ...
    function travelToDimension(fromDimension, toDimension, x, y, z) {
      if (
        !Donkeycraft.Dimensions ||
        !Donkeycraft.Dimensions.transformCoordinates
      ) {
        Donkeycraft.Logger.error(
          'Portal',
          'Dimensions system not available for travel'
        );
        return null;
      }
      ...
      var destPortal = findMatchingPortal(
        transformed.x,
        transformed.y,
        transformed.z,
        toDimension
      ...
      if (destPortal) {
        ...
        if (toDimension === Donkeycraft.DimensionType.NETHER) {
          var netherHeight = _findNetherSurfaceY(
      ...
      if (
        Donkeycraft.Dimensions &&
        Donkeycraft.Dimensions.setCurrentDimension
      ) {
