...
(function () {
   ...
   */
  Donkeycraft.Dimension = function (
    type,
    name,
    height,
    minY,
  ...
  Donkeycraft.Dimensions = (function () {
    ...
    function init() {
      ...
      _dimensions[Donkeycraft.DimensionType.OVERWORLD] =
        new Donkeycraft.Dimension(
      ...
      _spawnPositions[Donkeycraft.DimensionType.OVERWORLD] = {
        x: 0,
        y: 64,
        z: 0,
      };
    ...
    function transformCoordinates(fromType, toType, x, y, z) {
      ...
      if (toType === Donkeycraft.DimensionType.NETHER) {
        ...
        fromType === Donkeycraft.DimensionType.NETHER &&
        toType === Donkeycraft.DimensionType.OVERWORLD
      ) {
        // Coming from Nether: multiply by 8
        scale = NETHER_SCALE;
      } else if (
        toType === Donkeycraft.DimensionType.END ||
        fromType === Donkeycraft.DimensionType.END
      ) {
    ...
    function _wireGenerationToChunkManager(chunkManager, dimensionType) {
      ...
      chunkManager.onChunkLoad = function (chunk) {
        ...
        switch (dimensionType) {
          ...
          default: // Overworld — delegate to ChunkManager's built-in generation
            if (chunkManager.generateChunkTerrain) {
              var genResult = chunkManager.generateChunkTerrain(
                chunk.chunkX,
                chunk.chunkZ
              );
        ...
        if (!terrainGenerated && chunkManager.generateChunkTerrain) {
          try {
            var fbResult = chunkManager.generateChunkTerrain(
              chunk.chunkX,
              chunk.chunkZ
            );
        ...
        if (
          Donkeycraft.LightingEngine &&
          Donkeycraft.LightingEngine.updateChunkLighting
        ) {
          try {
          ...
          if (
            ...
            try {
              ...
              if (dim && !dim.hasSkyLight) {
                ...
                Donkeycraft.LightingEngine.calculateBlockLight(
                  dirtyChunks[i].chunk
                );
              } else {
                Donkeycraft.LightingEngine.updateChunkLighting(
