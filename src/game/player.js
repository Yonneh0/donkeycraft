...
(function () {
   ...
   */
  Donkeycraft.Player.findLandSpawnPosition = function (
    chunkManager,
    startX,
    startZ,
    options
    ...
    var maxChunkRings =
      options.maxChunkRings !== undefined ? options.maxChunkRings : 128;
    ...
    function isSolid(id) {
      return (
        id > 0 && Donkeycraft.BlockTypes && Donkeycraft.BlockTypes.isSolid(id)
      );
    }
    ...
    function isLiquid(id) {
      return (
        id > 0 && Donkeycraft.BlockTypes && Donkeycraft.BlockTypes.isLiquid(id)
      );
    }
    ...
    function checkNotOverWater(x, z, surfaceY) {
      ...
      for (var wy = surfaceY + 1; wy <= surfaceY + 2; wy++) {
        ...
        if (isLiquid(wBlock) && isLiquid(getBlockAt(x, wy - 1, z)))
          return false;
      }
      return true;
    }
    ...
    function checkClearPlayerSpace(x, z, feetY) {
      ...
      for (
        var cy = Math.floor(feetY);
        cy <= Math.floor(feetY + playerHeight);
        cy++
      ) {
    ...
    for (
      var phaseRing = 0;
      phaseRing <= maxChunkRings && chunksChecked < maxTotalChunksChecked;
      phaseRing++
    ) {
      ...
      for (
        var pcx = pStartX;
        pcx <= pEndX && chunksChecked < maxTotalChunksChecked;
        pcx++
      ) {
        var pcz = pStartZ;
        var pkey = pcx + ',' + pcz;
      ...
      for (
        pcz = pStartZ + 1;
        pcz <= pEndZ && chunksChecked < maxTotalChunksChecked;
        pcz++
      ) {
        var prx = pEndX;
        var prkey = prx + ',' + pcz;
      ...
      for (
        var pbx = pEndX - 1;
        pbx >= pStartX && chunksChecked < maxTotalChunksChecked;
        pbx--
      ) {
        var pbz = pEndZ;
        var pbkey = pbx + ',' + pbz;
      ...
      for (
        var plz = pEndZ - 1;
        plz >= pStartZ + 1 && chunksChecked < maxTotalChunksChecked;
        plz--
      ) {
        var plx = pStartX;
        var plkey = plx + ',' + plz;
    ...
    while (ring <= maxChunkRings && chunksChecked < maxTotalChunksChecked) {
      ...
      for (
        var cx = ringStartX;
        cx <= ringEndX && chunksChecked < maxTotalChunksChecked;
        cx++
      ) {
        var cz = ringStartZ;
        var key = cx + ',' + cz;
      ...
      for (
        cz = ringStartZ + 1;
        cz <= ringEndZ && chunksChecked < maxTotalChunksChecked;
        cz++
      ) {
        var crx = ringEndX;
        var rkey = crx + ',' + cz;
      ...
      for (
        var bcx = ringEndX - 1;
        bcx >= ringStartX && chunksChecked < maxTotalChunksChecked;
        bcx--
      ) {
        var bcz = ringEndZ;
        var bkey = bcx + ',' + bcz;
      ...
      for (
        var lcz = ringEndZ - 1;
        lcz >= ringStartZ + 1 && chunksChecked < maxTotalChunksChecked;
        lcz--
      ) {
        var lcx = ringStartX;
        var lkey = lcx + ',' + lcz;
  ...
  Donkeycraft.Player.prototype.adjustYaw = function (deltaYaw) {
    var twoPi = Math.PI * 2;
    this._rotation.yaw =
      (((this._rotation.yaw + deltaYaw) % twoPi) + twoPi) % twoPi;
  };
  ...
  Donkeycraft.Player.prototype.calculateFallDamage = function (fallDistance) {
    ...
    var damage =
      (fallDistance - Config.FALL_DAMAGE_THRESHOLD) *
      Config.FALL_DAMAGE_MULTIPLIER;
    return damage;
  };
   ...
   */
  Donkeycraft.Player.prototype.applyKnockback = function (
    direction,
    strength,
    upwardForce
  ) {
