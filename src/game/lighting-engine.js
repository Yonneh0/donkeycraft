...
(function () {
  ...
  Donkeycraft.LightingEngine = (function () {
    ...
    function _getBlockLightOpacity(blockId) {
      ...
      try {
        if (
          Donkeycraft.BlockRegistry.isReplaceable &&
          Donkeycraft.BlockRegistry.isReplaceable(blockId)
        )
          return 0;
        if (
          Donkeycraft.BlockRegistry.isLiquid &&
          Donkeycraft.BlockRegistry.isLiquid(blockId)
        )
          return 1;
        if (
          Donkeycraft.BlockRegistry.isTransparent &&
          Donkeycraft.BlockRegistry.isTransparent(blockId)
        )
          return 1;
        if (
          Donkeycraft.BlockRegistry.isSolid &&
          Donkeycraft.BlockRegistry.isSolid(blockId)
        )
          return 15;
      } catch (e) {
        /* ignore registry method errors */
      }
        ...
        for (var x = 0; x < CHUNK_SIZE; x++) {
          for (var z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (var y = surfaceY; y >= 0 && y < WORLD_HEIGHT; y--) {
              ...
              var lightAtDepth = Math.max(
                0,
                Math.round(15 * Math.pow(0.85, depthBelowSurface))
              );
    ...
    function updateBlockLighting(chunk, x, y, z) {
      ...
      var blockId = chunk.getBlock(x, y, z);
      if (
        _getLightEmission(blockId) > 0 ||
        (y + 1 < WORLD_HEIGHT &&
          _getLightEmission(chunk.getBlock(x, y + 1, z)) > 0)
      ) {
