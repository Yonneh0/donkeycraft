...
(function () {
  ...
  Donkeycraft.NavMesh.prototype._updateCellWalkability = function (cx, cy, cz) {
    ...
    if (
      Donkeycraft.Block &&
      typeof Donkeycraft.Block.isTransparent === 'function'
    ) {
      isSolid = !Donkeycraft.Block.isTransparent(blockId);
    }

    var blockBelowId = this._getBlockId(
      Math.floor(wx),
      Math.floor(wy) - 1,
      Math.floor(wz)
    );
   ...
   */
  Donkeycraft.NavMesh.prototype.getWalkableNeighbors = function (
    cx,
    cy,
    cz,
    includeVertical
  ) {
    var dirs = includeVertical
      ? Donkeycraft.AIAllDirections
      : Donkeycraft.AICardinalDirections;
    var neighbors = [];
   ...
   */
  Donkeycraft.NavMesh.prototype.findPath = function (
    sx,
    sy,
    sz,
    ex,
   ...
   * @private
   */
  Donkeycraft.AStarPathfinder.prototype._heuristic = function (
    ax,
    ay,
    az,
    bx,
   ...
   * @private
   */
  Donkeycraft.AStarPathfinder.prototype._getNeighbors = function (
    node,
    includeVertical
  ) {
    var dirs = includeVertical
   ...
   * @returns {{path: Array, steps: number, found: boolean}|null} Path result.
   */
  Donkeycraft.AStarPathfinder.prototype.findPath = function (
    startX,
    startY,
    startZ,
    endX,
    ...
    while (openSet.length > 0 && steps < maxSteps) {
      ...
      for (var j = 0; j < neighborList.length; j++) {
        ...
        if (!openSet.includes(neighborNode)) {
          ...
          neighborNode.h = this._heuristic(
            neighborNode.x,
            neighborNode.y,
            neighborNode.z,
            ex,
  ...
  Donkeycraft.MobAI.hasLineOfSight = function (
    x1,
    y1,
    z1,
    x2,
  ...
  var _followPathToTarget = function (context) {
    ...
    try {
      var vel = ai.calculateMovementVelocity(
        pos.x,
        pos.z,
        targetX,
        targetZ,
  ...
  Donkeycraft.AIStateMachine.prototype.tick = function (deltaTime) {
    ...
    try {
      ...
      console.warn(
        '[AIStateMachine] State update threw an error in state "' +
  ...
  Donkeycraft.IdleState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.IdleState.prototype.update = function (deltaTime) {
    ...
    if (this._lookTimer >= this._lookInterval) {
      ...
      if (entity && entity.setRotation) {
        ...
        var newYaw = currentRot
          ? currentRot.yaw + (Math.random() - 0.5) * 0.5
          : 0;
    ...
    if (
      this.ai.hasDamage() &&
      profile.fleeDistance > 0 &&
      this.ai.getHealthRatio() < 0.3
    ) {
      var nearestDist = this.ai.getNearestEnemyDistance(
        profile.chaseDistance || 20
      );
  ...
  Donkeycraft.WanderState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.WanderState.prototype.update = function (deltaTime) {
    ...
    if (
      this._pathRecalcTimer >= 1.0 ||
      !this._currentPath ||
      !this._currentPath.path ||
      this._currentPath.path.length === 0
    ) {
  ...
  Donkeycraft.WanderState.prototype._recalcWanderPath = function () {
    ...
    if (profile && profile.smartPathfinding && this.ai.navMesh) {
      try {
        var result = this.ai.navMesh.findPath(
          pos.x,
          pos.y,
          pos.z,
          endX,
          endY,
          endZ,
          Donkeycraft.AStarDefaults.WANDER_MAX_STEPS
        );
  ...
  Donkeycraft.ChaseState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.ChaseState.prototype.update = function (deltaTime) {
    ...
    if (
      ...
      dist <= Donkeycraft.AIDistances.ATTACK_RANGE
    ) {
      return Donkeycraft.AIState.ATTACK;
    }

    if (
      profile.retreatDistance &&
      profile.retreatDistance > 0 &&
      this.ai.getHealthRatio() < 0.3
    ) {
    ...
    if (profile && profile.smartPathfinding && this.ai.navMesh) {
      try {
        var result = this.ai.navMesh.findPath(
          ...
          endY,
          endZ,
          Donkeycraft.AStarDefaults.CHASE_MAX_STEPS
        );
        if (result && result.path && result.path.length > 0) {
          this._currentPath = result;
          return;
  ...
  Donkeycraft.FleeState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.FleeState.prototype.update = function (deltaTime) {
    ...
    if (
      dist >
        (profile.fleeDistance || 10) *
          Donkeycraft.AIDistances.FLEE_SAFETY_MARGIN &&
      this.ai.getHealthRatio() > 0.5
    ) {
      return Donkeycraft.AIState.IDLE;
    }
  ...
  Donkeycraft.FleeState.prototype._recalcFleePath = function (dangerPos) {
    ...
    if (dist > 0.1) {
      var fleeDistance =
        (profile && profile.fleeDistance ? profile.fleeDistance : 8) +
        Math.random() * 5;
    ...
    if (this.ai.navMesh) {
      try {
        var result = this.ai.navMesh.findPath(
          ...
          fleeY,
          fleeZ,
          Donkeycraft.AStarDefaults.FLEE_MAX_STEPS
        );
        if (result && result.path && result.path.length > 0) {
          this._currentPath = result;
          return;
  ...
  Donkeycraft.AmbushState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
    ...
    if (this.ai.navMesh) {
      try {
        var result = this.ai.navMesh.findPath(
          ...
          targetPos.y,
          targetZ,
          Donkeycraft.AStarDefaults.AMBUSH_MAX_STEPS
        );
        if (result && result.path && result.path.length > 0) {
          this._currentPath = result;
          return;
  ...
  Donkeycraft.AttackState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.AttackState.prototype.update = function (deltaTime) {
    ...
    if (
      profile.rangedDistance &&
      dist <= profile.rangedDistance &&
      dist > Donkeycraft.AIDistances.ATTACK_RANGE
    ) {
      this._attackTimer += deltaTime;
  ...
  Donkeycraft.AttackState.prototype._performAttack = function (target) {
    ...
    if (
      Donkeycraft.EventBus &&
      typeof Donkeycraft.EventBus.emitSafe === 'function'
    ) {
      try {
  ...
  Donkeycraft.AttackState.prototype._performRangedAttack = function (target) {
    ...
    if (target.takeDamage && typeof target.takeDamage === 'function') {
      target.takeDamage(
        damage,
        this.ai.entity ? this.ai.entity.type : 'skeleton'
      );
    }
    ...
    if (
      ...
      try {
        ...
        console.warn(
          '[AttackState] Failed to emit entity:ranged_attack event:',
          e
        );
      }
    }
  ...
  Donkeycraft.AttackState.prototype._recalcAttackPath = function (targetPos) {
    ...
    var keepDist =
      this.ai.profile.keepDistance || Donkeycraft.AIDistances.ATTACK_RANGE;
    var dx = targetPos.x - pos.x;
    var dz = targetPos.z - pos.z;
    var dist = Math.sqrt(dx * dx + dz * dz);
    ...
    if (this.ai.navMesh) {
      try {
        var result = this.ai.navMesh.findPath(
          ...
          endY,
          endZ,
          Donkeycraft.AStarDefaults.ATTACK_MAX_STEPS
        );
        if (result && result.path && result.path.length > 0) {
          this._currentPath = result;
          return;
  ...
  Donkeycraft.FollowState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.PatrolState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
    ...
    if (pos) {
      ...
      if (dx * dx + dy * dy + dz * dz < 2.0) {
        this._currentPointIndex =
          (this._currentPointIndex + 1) % this._patrolPoints.length;
        this._pointReachedTimer = 0;
      }
    }
    ...
    if (this.ai.navMesh) {
      try {
        var result = this.ai.navMesh.findPath(
          ...
          targetPoint.y,
          targetPoint.z,
          Donkeycraft.AStarDefaults.PATROL_MAX_STEPS
        );
        if (result && result.path && result.path.length > 0) {
          this._currentPath = result;
          return;
  ...
  Donkeycraft.HurtState.prototype = Object.create(
    Donkeycraft.AIStateInstance.prototype
  );
  ...
  Donkeycraft.AIComponent.prototype.setProfile = function (profile) {
    if (typeof profile === 'string') {
      ...
      if (namedProfile) {
        ...
        this.profile = JSON.parse(
          JSON.stringify(Donkeycraft.AIBehaviorProfiles.generic)
        );
      }
    ...
    var hurtState =
      this.stateMachine._states &&
      this.stateMachine._states[Donkeycraft.AIState.HURT];
  ...
  Donkeycraft.AIComponent.prototype.getNearestEnemyDistance = function (range) {
    ...
    if (Donkeycraft._entityManager) {
      try {
        var nearbyEntities = Donkeycraft._entityManager.getEntitiesInRange(
          pos.x,
          pos.y,
          pos.z,
          range
        ...
        for (var i = 0; i < nearbyEntities.length; i++) {
          ...
          if (
            entityProfile.attackDamage > 0 ||
            entityProfile.chaseDistance > 0
          ) {
   ...
   */
  Donkeycraft.AIComponent.prototype.calculateMovementVelocity = function (
    mobX,
    mobZ,
    targetX,
    targetZ,
  ...
  Donkeycraft.AIComponent.prototype.tick = function (deltaTime) {
    ...
    if (
      hurtState &&
      typeof hurtState.setRecoveryState === 'function' &&
      currentState !== Donkeycraft.AIState.HURT
    ) {
    ...
    if (
      currentState === Donkeycraft.AIState.IDLE &&
      newState !== Donkeycraft.AIState.IDLE
    ) {
      this.idleTimer = 0;
    }
    ...
    if (this.entity && this.entity._animationController) {
      ...
      if (vel && pos) {
        var speed = Math.sqrt(
          (vel.x || 0) * (vel.x || 0) + (vel.z || 0) * (vel.z || 0)
        );
        ...
        switch (stateName) {
          ...
          case Donkeycraft.AIState.WANDER:
            this.entity._animationController.setSpeed(
              Donkeycraft.AISpeeds.WANDER,
              yaw
            );
            break;
          case Donkeycraft.AIState.PATROL:
            this.entity._animationController.setSpeed(
              Donkeycraft.AISpeeds.PATROL,
              yaw
            );
            break;
          case Donkeycraft.AIState.FOLLOW:
            this.entity._animationController.setSpeed(
              Donkeycraft.AISpeeds.FOLLOW,
              yaw
            );
            break;
          case Donkeycraft.AIState.FLEE:
            this.entity._animationController.setSpeed(
              Donkeycraft.AISpeeds.FLEE,
              yaw
            );
            break;
          case Donkeycraft.AIState.CHASE:
            this.entity._animationController.setSpeed(
              Donkeycraft.AISpeeds.CHASE,
              yaw
            );
            break;
          case Donkeycraft.AIState.AMBUSH:
            this.entity._animationController.setSpeed(
              Donkeycraft.AISpeeds.AMBUSH,
              yaw
            );
            break;
   ...
   */
  Donkeycraft.MobAI.calculateChaseVelocity = function (
    mobX,
    mobZ,
    targetX,
    targetZ,
   ...
   * @returns {{vx: number, vz: number}} Velocity components.
   */
  Donkeycraft.MobAI.calculateFleeVelocity = function (
    mobX,
    mobZ,
    sourceX,
    sourceZ,
   ...
   */
  Donkeycraft.MobAI.findPath = function (
    startX,
    startY,
    startZ,
    endX,
