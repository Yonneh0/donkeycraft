...
(function () {
   ...
   */
  Donkeycraft.RaycastResult = function (
    blockX,
    blockY,
    blockZ,
    faceNormalX,
  ...
  Donkeycraft.Raycast = (function () {
    ...
    function raycast(chunkManager, origin, direction, reach, ignoreIds) {
      ...
      if (startBlockId !== 0 && !_shouldIgnore(startBlockId)) {
        ...
        // When the ray origin is inside a solid block, compute an approximate
        // face normal by testing which adjacent face the origin is closest to
        // crossing. This prevents zero-normal issues downstream (block placement,
        // HUD highlighting). We return the face that would be hit if the player
        // moved even slightly in the ray direction — i.e., the first axis where
        // tMax is smallest indicates the face we're about to exit through.
        var approxNormalX = 0;
        var approxNormalY = 0;
        var approxNormalZ = 0;
        if (tMaxX <= tMaxY && tMaxX <= tMaxZ) {
          approxNormalX = dirX > 0 ? 1 : -1;
        } else if (tMaxY <= tMaxX && tMaxY <= tMaxZ) {
          approxNormalY = dirY > 0 ? 1 : -1;
        } else {
          approxNormalZ = dirZ > 0 ? 1 : -1;
        }
