  ...
  Donkeycraft.BlockStateRegistry = (function () {
    ...
    function _initStates() {
      ...
      for (var l = 0; l < logNames.length; l++) {
        ...
        if (lid !== -1) {
          registerStates(lid, [{ axis: 'y' }, { axis: 'x' }, { axis: 'z' }]);
        }
      }
      ...
      for (var w = 0; w < woolColors.length; w++) {
        ...
        if (woolId !== -1) {
          _blockStates[woolId] = [
            new Donkeycraft.BlockState({ color: woolColors[w] }),
          ];
          _stateIndex[woolId + ':color=' + woolColors[w]] =
            new Donkeycraft.BlockState({ color: woolColors[w] });
        }
      }
      ...
      for (var c1 = 0; c1 < 16; c1++) {
        ...
        if (concreteId !== -1) {
          _blockStates[concreteId] = [
            new Donkeycraft.BlockState({ color: woolColors[c1] }),
          ];
          _stateIndex[concreteId + ':color=' + woolColors[c1]] =
            new Donkeycraft.BlockState({ color: woolColors[c1] });
        }
      }
      ...
      for (var c2 = 0; c2 < 16; c2++) {
        ...
        if (glassId !== -1) {
          _blockStates[glassId] = [
            new Donkeycraft.BlockState({ color: woolColors[c2] }),
          ];
          _stateIndex[glassId + ':color=' + woolColors[c2]] =
            new Donkeycraft.BlockState({ color: woolColors[c2] });
        }
      }
      ...
      for (var c3 = 0; c3 < 16; c3++) {
        ...
        if (powderId !== -1) {
          _blockStates[powderId] = [
            new Donkeycraft.BlockState({ color: woolColors[c3] }),
          ];
          _stateIndex[powderId + ':color=' + woolColors[c3]] =
            new Donkeycraft.BlockState({ color: woolColors[c3] });
        }
      }
      ...
      if (redstoneTorchId !== -1) {
        registerStates(redstoneTorchId, [{ lit: 'true' }, { lit: 'false' }]);
      }
      ...
      for (var ss = 0; ss < slabNames.length; ss++) {
        ...
        if (slabId !== -1) {
          registerStates(slabId, [{ half: 'lower' }, { half: 'upper' }]);
        }
      }
    ...
    function hasStates(blockId) {
      return (
        _blockStates[blockId] !== undefined && _blockStates[blockId].length > 0
      );
    }
