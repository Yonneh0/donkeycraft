  ...
  Donkeycraft.RedstoneTNT = (function () {
    ...
    function _processTNT(entry, chunk) {
      ...
      if (state.fuseTicks <= 0) {
        ...
        Donkeycraft.RedstoneEngine.markDirtyRange(
          entry.x - EXPLOSION_RADIUS,
          entry.y - EXPLOSION_RADIUS,
          entry.z - EXPLOSION_RADIUS,
          entry.x + EXPLOSION_RADIUS,
          entry.y + EXPLOSION_RADIUS,
          entry.z + EXPLOSION_RADIUS
        );
      ...
      for (var i = 0; i < neighbors.length; i++) {
        ...
        if (nBlockId === 173 || nBlockId === 229) {
          ...
          if (
            ...
            var signal = Donkeycraft._redstoneWiring.getSignalStrength(
              nx,
              ny,
              nz
            );
    ...
    function _litTNT(x, y, z) {
      ...
      if (!_tntStates[key]) {
        _tntStates[key] = {
          ...
          lit: true,
          primeTick: Donkeycraft.RedstoneEngine
            ? Donkeycraft.RedstoneEngine.getCurrentTick()
            : 0,
        };
      } else {
        ...
        _tntStates[key].primeTick = Donkeycraft.RedstoneEngine
          ? Donkeycraft.RedstoneEngine.getCurrentTick()
          : 0;
      }
    ...
    function _explode(x, y, z) {
      ...
      for (var i = 0; i < blocksToCheck.length; i++) {
        ...
        _blastWaves[chunkKey][
          by * CHUNK_SIZE * CHUNK_SIZE + bLocalX * CHUNK_SIZE + bLocalZ
        ] = blastStrength;
      }
