    ...
    Donkeycraft.RedstoneWiring = (function() {
        ...
        function setSignalStrength(x, y, z, strength) {
            ...
            if (!_signalStrengths[chunkKey]) {
                _signalStrengths[chunkKey] = new Uint8Array(CHUNK_SIZE * WORLD_HEIGHT * CHUNK_SIZE);
            }
        ...
        function _processDirtyWire(entry, chunk) {
            ...
            var newStrength = _calculateWireStrength(entry.x, entry.y, entry.z, chunk);

            var chunkKey = Donkeycraft.WorldUtils.makeChunkKey(entry.chunkX, entry.chunkZ);
            if (!_signalStrengths[chunkKey]) {
                _signalStrengths[chunkKey] = new Uint8Array(CHUNK_SIZE * WORLD_HEIGHT * CHUNK_SIZE);
            }

            var idx = entry.y * CHUNK_SIZE * CHUNK_SIZE + localX * CHUNK_SIZE + localZ;
            ...
            if (newStrength !== oldStrength) {
                ...
                if (Donkeycraft.RedstoneEngine && Donkeycraft.RedstoneEngine.setEventBus) {
            ...
            for (var i = 0; i < neighbors.length; i++) {
                ...
                if (WIRE_BLOCK_IDS[nBlockId]) {
                    ...
                    if (_signalStrengths[nChunkKey]) {
                        var nIdx = ny * CHUNK_SIZE * CHUNK_SIZE + nLocalX * CHUNK_SIZE + nLocalZ;
        ...
        function _processTorch(entry, chunk) {
            ...
            var belowBlockId = belowChunk.getBlock(belowLocalX, entry.y - 1, belowLocalZ);

            // Check if below block is powered (has signal strength > 0)
            var belowPowered = false;
            if (WIRE_BLOCK_IDS[belowBlockId]) {
                var belowChunkKey = Donkeycraft.WorldUtils.makeChunkKey(belowChunkX, belowChunkZ);
                if (_signalStrengths[belowChunkKey]) {
                    var belowIdx = (entry.y - 1) * CHUNK_SIZE * CHUNK_SIZE + belowLocalX * CHUNK_SIZE + belowLocalZ;
            ...
            _torchStates[key] = {
                powered: newState,
                tick: Donkeycraft.RedstoneEngine ? Donkeycraft.RedstoneEngine.getCurrentTick() : 0
            };
            ...
            for (var i = 0; i < lampOffsets.length; i++) {
                ...
                if (lBlockId === REDSTONE_LAMP || lBlockId === LIT_REDSTONE_LAMP) {
                    ...
                    if (shouldLit !== currentLit) {
                        // Set new block state
                        lChunk.setBlock(lLocalX, ly, lLocalZ, shouldLit ? LIT_REDSTONE_LAMP : REDSTONE_LAMP);
        ...
        function _queueAdjacentDirty(x, y, z) {
            ...
            for (var i = 0; i < offsets.length; i++) {
                var off = offsets[i];
                Donkeycraft.RedstoneEngine.markDirty(x + off.dx, y + off.dy, z + off.dz);
            }
        }
