...
(function () {
   ...
   */
  Donkeycraft.ChunkManager.prototype.getChunkIfExists = function (
    chunkX,
    chunkZ
  ) {
   ...
   * @param {number} chunkZ - Chunk Z coordinate.
   */
  Donkeycraft.ChunkManager.prototype.markChunkDirty = function (
    chunkX,
    chunkZ
  ) {
    var key = chunkKey(chunkX, chunkZ);
   ...
   * @param {number} chunkZ - Chunk Z coordinate.
   */
  Donkeycraft.ChunkManager.prototype.markChunkClean = function (
    chunkX,
    chunkZ
  ) {
   ...
   */
  Donkeycraft.ChunkManager.prototype.updatePlayerPosition = function (
    playerChunkX,
    playerChunkZ
  ) {
   ...
   */
  Donkeycraft.ChunkManager.prototype.updateRenderDistance = function (
    newRadius
  ) {
   ...
   */
  Donkeycraft.ChunkManager.prototype.generateChunkTerrain = function (
    chunkX,
    chunkZ
  ) {
        ...
        if (
          ...
          try {
            ...
            Donkeycraft.Logger.error(
              'ChunkManager',
              'StructureGenerator failed: ' + e.message
            );
  ...
  function _generateOverworldChunk(manager, chunk, chunkX, chunkZ) {
    ...
    if (Donkeycraft.BiomeRegistry && Donkeycraft.PerlinNoise) {
      ...
      var tempNoise = Donkeycraft.PerlinNoise.noise2D(
        worldX * 0.005,
        worldZ * 0.005
      );
      var rainNoise = Donkeycraft.PerlinNoise.noise2D(
        worldX * 0.005 + 1000,
        worldZ * 0.005 + 1000
      );
      // Map noise [-1, 1] → [0, 1]
    ...
    if (!biome) {
      biome = Donkeycraft.BiomeRegistry.getBiomeById(
        Donkeycraft.BiomeID.PLAINS
      );
    }
    ...
    try {
      ...
      Donkeycraft.Logger.error(
        'ChunkManager',
        'Heightmap generation failed: ' + e.message
      );
      return;
    }
    ...
    if (!heightmap || heightmap.length === 0) {
      Donkeycraft.Logger.error(
        'ChunkManager',
        'Empty heightmap for chunk [' + chunkX + ',' + chunkZ + ']'
      );
      return;
    }
    ...
    if (!bedrock || !stone || !dirt) {
      Donkeycraft.Logger.error(
        'ChunkManager',
        'Required blocks not found in BlockRegistry'
      );
      return;
    }
    ...
    if (Donkeycraft.CaveGenerator && Donkeycraft.CaveGenerator.generateCaves) {
      try {
        Donkeycraft.CaveGenerator.generateCaves(chunk, chunk.biomeId);
      } catch (e) {
        /* skip */
      }
    }
    ...
    if (Donkeycraft.OreGenerator && Donkeycraft.OreGenerator.placeOres) {
      try {
        Donkeycraft.OreGenerator.placeOres(chunk, chunk.biomeId);
      } catch (e) {
        /* skip */
      }
    }
    ...
    if (Donkeycraft.WaterGenerator && Donkeycraft.WaterGenerator.placeWater) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'ChunkManager',
          'Water placement failed for chunk [' +
            chunkX +
            ',' +
            chunkZ +
            ']: ' +
    ...
    if (
      ...
      try {
        ...
        Donkeycraft.Logger.warn(
          'ChunkManager',
          'Surface layer application failed for chunk [' +
            chunkX +
            ',' +
            chunkZ +
            ']: ' +
  ...
  function _buildChunkSurfaceMap(chunk) {
    ...
    for (var lx = 0; lx < CHUNK_SIZE; lx++) {
      ...
      for (var lz = 0; lz < CHUNK_SIZE; lz++) {
        ...
        map[lx][lz] =
          surfaceY >= 0 ? { y: surfaceY, blockId: surfaceBlockId } : null;
      }
    }
  ...
  function _generateNetherChunk(manager, chunk, chunkX, chunkZ) {
    if (
      !Donkeycraft.NetherGenerator ||
      !Donkeycraft.NetherGenerator.generateNetherTerrain
    ) {
    ...
    try {
      ...
      Donkeycraft.Logger.error(
        'ChunkManager',
        'Nether terrain generation failed: ' + e.message
      );
    }
  ...
  function _generateEndChunk(manager, chunk, chunkX, chunkZ) {
    if (
      !Donkeycraft.EndGenerator ||
      !Donkeycraft.EndGenerator.generateEndTerrain
    ) {
    ...
    try {
      ...
      Donkeycraft.Logger.error(
        'ChunkManager',
        'End terrain generation failed: ' + e.message
      );
    }
