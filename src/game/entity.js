...
(function () {
  ...
  Donkeycraft.Entity = function (config) {
    ...
    var defaultY =
      Donkeycraft.Config && Donkeycraft.Config.WORLD_HEIGHT
        ? Donkeycraft.Config.WORLD_HEIGHT / 2
        : 64;
    ...
    this.health =
      config.health !== undefined
        ? Math.min(config.health, this.maxHealth)
        : this.maxHealth;
    ...
    this.showNametag =
      config.showNametag !== undefined ? !!config.showNametag : true;
  ...
  Donkeycraft.Entity.prototype._initAnimationSystem = function () {
    ...
    if (
      this.bones &&
      Array.isArray(this.bones) &&
      this._animationController &&
      this._animationController.getState
    ) {
      return;
    }

    var typeDef = Donkeycraft.EntityTypeDB
      ? Donkeycraft.EntityTypeDB[this.type]
      : null;
  ...
  Donkeycraft.Entity.prototype._createAnimationController = function (typeDef) {
    ...
    if (typeDef && typeDef.animations && Array.isArray(typeDef.animations)) {
      ...
      for (var i = 0; i < typeDef.animations.length; i++) {
        ...
        var animDef = Donkeycraft.AnimationDefinitions
          ? Donkeycraft.AnimationDefinitions[animName]
          : null;
        if (animDef) {
  ...
  Donkeycraft.Entity.prototype.setRotation = function (yaw, pitch) {
    ...
    var clampFn =
      typeof Donkeycraft.clamp === 'function' ? Donkeycraft.clamp : null;
  ...
  Donkeycraft.Entity.prototype.takeDamage = function (amount, source) {
    ...
    if (
      ...
      Donkeycraft.AnimationDefinitions.hurt
    ) {
    ...
    for (var i = 0; i < this._subscribers.length; i++) {
      try {
        ...
        if (
          ...
          console.error(
            '[Entity] Tick subscriber error for entity type "' +
              this.type +
              '" (id=' +
              this._id +
              '):',
  ...
  Donkeycraft.Entity.prototype._resolvePhysicsCollision = function (deltaTime) {
    ...
    if (!collision || typeof collision.resolveMovementWithDelta !== 'function')
      return;
