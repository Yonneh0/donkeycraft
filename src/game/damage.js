...
(function () {
     ...
     * In vanilla Minecraft, each point of damage = 1 HP (half a heart).
    ...
    Donkeycraft.HurtBox.prototype.calculateFallDamage = function (fallDistance) {
        ...
        // No fall damage for first 3 blocks of free fall (vanilla Minecraft behavior)
        if (fallDistance <= Config.FALL_DAMAGE_THRESHOLD) {
            return 0;
        }

        // Damage = (fallDistance - threshold) × FALL_DAMAGE_MULTIPLIER HP per block
        // Each point of damage = 1 HP (half a heart), matching vanilla Minecraft
     ...
     * Damage is applied via `takeDamage()`, which respects game mode immunity (creative = immune).
