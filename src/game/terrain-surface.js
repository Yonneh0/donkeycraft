...
(function () {
  ...
  function _resolveSurfaceBlocks() {
    ...
    var blockVariants = [
      ...
      {
        key: 'snow_block',
        variants: ['snow_block', 'snowBlock', 'snow_layer', 'snow'],
      },
  ...
  function _buildSurfaceConfigurations() {
    ...
    _surfaceLayers['grass'] = [
      new SurfaceLayer('grass_top', _resolvedBlocks['grass_block'] || 0, 0, 0, [
        'grass',
        'forest',
      ]),
      new SurfaceLayer('dirt_sub', _resolvedBlocks['dirt'] || 0, 1, 3, [
        'grass',
        'forest',
      ]),
      new SurfaceLayer(
        'stone_deep',
        _resolvedBlocks['stone'] || 0,
        4,
        WORLD_HEIGHT,
        ['grass', 'forest']
      ),
    ...
    var arcticSnowId =
      _resolvedBlocks['snow_block'] ||
      _resolvedBlocks['snowBlock'] ||
      _resolvedBlocks['stone'];
    var arcticIceId =
      _resolvedBlocks['packed_ice'] ||
      _resolvedBlocks['ice'] ||
      _resolvedBlocks['stone'];
    _surfaceLayers['arctic'] = [
      new SurfaceLayer(
        'snow_top',
        arcticSnowId || _resolvedBlocks['stone'] || 1,
        0,
        0,
        ['arctic']
      ),
      new SurfaceLayer(
        'ice_sub',
        arcticIceId || _resolvedBlocks['stone'] || 1,
        1,
        2,
        ['arctic']
      ),
    ...
    _surfaceLayers['desert'] = [
      new SurfaceLayer('sand_top', _resolvedBlocks['sand'] || 0, 0, 2, [
        'desert',
      ]),
      new SurfaceLayer(
        'sandstone_sub',
        _resolvedBlocks['sandstone'] || 0,
        3,
        10,
        ['desert']
      ),
      new SurfaceLayer(
        'desert_stone_deep',
        _resolvedBlocks['desert_stone'] || _resolvedBlocks['stone'] || 0,
        11,
        WORLD_HEIGHT,
        ['desert']
      ),
    ...
    _surfaceLayers['forest'] = [
      ...
      new SurfaceLayer(
        'rich_dirt_1',
        _resolvedBlocks['rich_dirt'] || _resolvedBlocks['dirt'] || 0,
        1,
        2,
        ['forest']
      ),
      new SurfaceLayer(
        'rich_dirt_2',
        _resolvedBlocks['topsoil'] || _resolvedBlocks['dirt'] || 0,
        3,
        4,
        ['forest']
      ),
  ...
  function applySurfaceLayers(chunk, chunkX, chunkZ, biomeName, heightmap) {
    ...
    if (!_initialized) {
      if (typeof console !== 'undefined' && console.warn) {
        console.warn(
          '[TerrainSurface] Not initialized yet, skipping surface layers for biome "' +
            biomeName +
            '"'
        );
      }
    ...
    if (!layers) {
      ...
      if (!layers) {
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            '[TerrainSurface] No surface layers configured for biome "' +
              biomeName +
              '", using defaults'
          );
        }
    ...
    for (var x = 0; x < CHUNK_SIZE; x++) {
      for (var z = 0; z < CHUNK_SIZE; z++) {
        ...
        if (!isFinite(surfaceY) || surfaceY < 1 || surfaceY >= WORLD_HEIGHT)
          continue;
        ...
        var maxLayerDepth = 0;
        for (var li = 0; li < layers.length; li++) {
          if (layers[li].maxDepth > maxLayerDepth)
            maxLayerDepth = layers[li].maxDepth;
        }
