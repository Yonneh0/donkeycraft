...
(function () {
   ...
   */
  Donkeycraft.EntityManager.prototype._updateSpatialHash = function (
    entityId,
    position
  ) {
    if (!position) return;
   ...
   * @param {number} entityId - Entity ID.
   */
  Donkeycraft.EntityManager.prototype._removeFromSpatialHash = function (
    entityId
  ) {
   ...
   */
  Donkeycraft.EntityManager.prototype._getNearbyEntityIds = function (
    cx,
    cy,
    cz,
    radius
   ...
   */
  Donkeycraft.EntityManager.prototype._updateAwareness = function (
    playerX,
    playerY,
    playerZ
  ) {
   ...
   */
  Donkeycraft.EntityManager.prototype._injectGroundCheck = function (
    groundCheckFn
  ) {
  ...
  Donkeycraft.EntityManager.prototype.getByType = function (type, maxResults) {
    ...
    for (
      var i = 0;
      i < list.length && result.length < (maxResults || 100);
      i++
    ) {
      if (list[i].isAlive()) {
        result.push(list[i]);
   ...
   */
  Donkeycraft.EntityManager.prototype.getEntitiesInRange = function (
    cx,
    cy,
    cz,
    range
        ...
        for (var cellX2 = minCellX2; cellX2 <= maxCellX2; cellX2++) {
          for (var cellZ2 = minCellZ2; cellZ2 <= maxCellZ2; cellZ2++) {
            ...
            cellSet2.forEach(function (entityId) {
              ...
              if (!entity2 || !entity2.isAlive() || !entity2.getPosition)
                return;
   ...
   */
  Donkeycraft.EntityManager.prototype.tick = function (
    deltaTime,
    playerX,
    playerY,
    playerZ,
