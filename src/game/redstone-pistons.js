    ...
    Donkeycraft.RedstonePistons = (function () {
        ...
        function _isPistonPowered(x, y, z, chunk) {
            ...
            for (var i = 0; i < sides.length; i++) {
                ...
                if (nBlockId === 174 || nBlockId === 176 || nBlockId === 230) { // torch, lit lamp, redstone block
                    return true;
                }
        ...
        function _tryExtend(x, y, z, state, isSticky) {
            ...
            var frontLocalX = (((x + pushDir.dx) % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE;
            var frontChunkZ = Math.floor((z + pushDir.dz) / CHUNK_SIZE);
            var frontLocalZ = (((z + pushDir.dz) % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE;
        ...
        function _calculatePushChain(px, py, pz, pushDir, isSticky) {
            ...
            if (endBlockId !== 0 && !Donkeycraft.BlockRegistry.isReplaceable(endBlockId)) {
