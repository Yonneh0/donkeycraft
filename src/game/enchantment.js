...
(function () {
   ...
   */
  Donkeycraft.Enchantment = function (
    id,
    name,
    maxLevel,
    weight,
  ...
  Donkeycraft.EnchantmentRegistry = (function () {
    ...
    function registerEnchantments() {
      ...
      ); // incompatible: Smite, BaneOfArthropods
      register(
        new Donkeycraft.Enchantment(2, 'Smite', 5, 5, ['weapon'], [1, 3], [], 1)
      ); // incompatible: Sharpness, BaneOfArthropods
      register(
        new Donkeycraft.Enchantment(
          3,
          'BaneOfArthropods',
      ...
      ); // incompatible: FireProtection, BlastProtection, ProjectileProtection
      register(
        new Donkeycraft.Enchantment(
          5,
          'FireProtection',
      ...
      ); // incompatible: Protection, BlastProtection, ProjectileProtection
      register(
        new Donkeycraft.Enchantment(
          6,
          'BlastProtection',
      ...
      ); // incompatible: Protection, FireProtection, ProjectileProtection
      register(
        new Donkeycraft.Enchantment(
          7,
      ...
      ); // incompatible: Protection, FireProtection, BlastProtection

      // Utility enchantments (both slots)
      register(
        new Donkeycraft.Enchantment(8, 'Fortune', 3, 2, ['weapon'], [], [], 1)
      ); // compatible with any damage enchant
      register(
        new Donkeycraft.Enchantment(
          9,
          'SilkTouch',
      ...
      register(
        new Donkeycraft.Enchantment(13, 'Power', 5, 10, ['weapon'], [], [], 1)
      ); // compatible with everything
      register(
        new Donkeycraft.Enchantment(14, 'Punch', 2, 3, ['weapon'], [], [], 1)
      ); // compatible with everything
      register(
        new Donkeycraft.Enchantment(
          15,
      ...
      register(
        new Donkeycraft.Enchantment(17, 'Thorns', 3, 2, ['armor'], [], [], 1)
      ); // compatible with everything
      register(
        new Donkeycraft.Enchantment(
          18,
      ...
      register(
        new Donkeycraft.Enchantment(19, 'Looting', 3, 2, ['weapon'], [], [], 1)
      ); // compatible with everything

      // Fishing rod enchantments
      register(
        new Donkeycraft.Enchantment(20, 'Lure', 3, 5, ['weapon'], [], [], 1)
      ); // compatible with everything
      register(
        new Donkeycraft.Enchantment(
          21,
      ...
      ); // incompatible: Loyalty, Riptide
      register(
        new Donkeycraft.Enchantment(26, 'Impaling', 5, 5, ['weapon'], [], [], 1)
      ); // compatible with everything
      ...
      ); // cursed, compatible with everything except other binding
      register(
        new Donkeycraft.Enchantment(
          31,
          'CurseOfVanishing',
      ...
      ); // cursed, compatible with everything
    ...
    function _getItemCategory(itemBlockId) {
      ...
      if ([250, 251, 252, 253, 254, 255].indexOf(itemBlockId) >= 0)
        return 'pickaxe'; // tool IDs
      if ([267, 268, 269, 270, 271, 272].indexOf(itemBlockId) >= 0)
        return 'bow'; // bow IDs
      if ([261, 262, 263, 264, 265, 266].indexOf(itemBlockId) >= 0)
        return 'fishing_rod';
      if ([273, 274, 275, 276, 277].indexOf(itemBlockId) >= 0) return 'trident';
      if ([278, 279, 280, 281, 282].indexOf(itemBlockId) >= 0)
        return 'crossbow';
    ...
    function applyEnchantment(itemStack, enchantId, level) {
      ...
      if (!enchant) {
        return {
          success: false,
          reason: 'Unknown enchantment ID: ' + enchantId,
        };
      }
      ...
      if (level < 1 || level > enchant.maxLevel) {
        return {
          success: false,
          reason:
            'Invalid level: ' + level + ' (max: ' + enchant.maxLevel + ')',
        };
      }
      ...
      if (!canApplyToItem(itemBlockId, enchantId)) {
        return {
          success: false,
          reason: 'Enchantment cannot be applied to this item',
        };
      }

      // Check compatibility with existing enchantments on the item
      var existingEnchants = itemStack.getEnchantments
      ...
      for (var existingId in existingEnchants) {
        if (existingEnchants.hasOwnProperty(existingId)) {
          if (!areCompatible(parseInt(existingId, 10), enchantId)) {
            var existingName = _enchantments[existingId]
              ? _enchantments[existingId].name
              : 'Unknown';
            return {
              success: false,
              reason: 'Incompatible with ' + existingName,
            };
          }
        }
      }
