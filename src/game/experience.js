...
(function () {
  ...
  Donkeycraft.Experience.prototype.setLevel = function (level) {
    ...
    var clampedLevel = Math.max(
      1,
      Math.min(Donkeycraft.Experience.MAX_LEVEL, Math.floor(level))
    );
  ...
  Donkeycraft.Experience.prototype.getLevelThreshold = function () {
    ...
    var nextTotal = Donkeycraft.Experience._computeTotalXPAtLevel(
      this._level + 1
    );
    var currentTotal = Donkeycraft.Experience._computeTotalXPAtLevel(
      this._level
    );
  ...
  Donkeycraft.Experience.prototype.addXP = function (amount) {
    ...
    while (
      this._level < Donkeycraft.Experience.MAX_LEVEL &&
      remaining >= threshold
    ) {
      remaining -= threshold;
      this._level++;
  ...
  Donkeycraft.Experience.prototype._recalcPointsFromTotalXP = function () {
    var currentLevelTotal = Donkeycraft.Experience._computeTotalXPAtLevel(
      this._level
    );
    var nextLevelTotal = Donkeycraft.Experience._computeTotalXPAtLevel(
      this._level + 1
    );
