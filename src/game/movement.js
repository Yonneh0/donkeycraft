...
(function () {
   ...
   */
  Donkeycraft.Movement.prototype._getSpeedDefinitions = function (
    gameMode,
    isFlying
  ) {
    if (isFlying) {
   ...
   */
  Donkeycraft.Movement.prototype._getSpeedForIndex = function (
    index,
    gameMode,
    terrainContext
  ) {
    ...
    if (cycleDownPressed && !this._speedKeyState.SPEED_CYCLE) {
      ...
      if (currentIdx < speeds.length - 1) {
        ...
        Donkeycraft.Logger.info(
          'Movement',
          'Speed increased to index ' + (currentIdx + 1) + ' via Shift'
        );
      } else {
        ...
        Donkeycraft.Logger.info(
          'Movement',
          'Speed cycled to index 0 (wrap-around) via Shift'
        );
      }
    ...
    if (cycleDown && !this._speedKeyState.SPEED_DOWN) {
      if (!isFlying) {
        ...
        if (currentIdx > 0) {
          ...
          Donkeycraft.Logger.info(
            'Movement',
            'Speed decreased to index ' + (currentIdx - 1) + ' via Z'
          );
        }
      }
    ...
    if (cycleUp && !this._speedKeyState.SPEED_UP) {
      if (!isFlying) {
        ...
        if (currentIdx < speeds.length - 1) {
          ...
          Donkeycraft.Logger.info(
            'Movement',
            'Speed increased to index ' + (currentIdx + 1) + ' via Q'
          );
        }
      }
      // Cooldown for key press detection
  ...
  Donkeycraft.Movement.prototype._detectTerrainEffects = function (x, y, z) {
    ...
    try {
      ...
      var footBlockId = this._chunkManager.getBlockId(
        footBlockX,
        footBlockY,
        footBlockZ
      );
      ...
      if (footBlockId === 224 || footBlockId === 241) {
        // mud, packed_mud
      ...
      if (footBlockId === 305) {
        // powder_snow (if it exists)
        return { slowModifier: Config.POWDER_SNOW_SLOW, type: 'powder_snow' };
      }
    } catch (e) {
      Donkeycraft.Logger.warn(
        'Movement',
        'Failed to detect terrain effects: ' + e.message
      );
    }
  ...
  Donkeycraft.Movement.prototype.tick = function (deltaTime) {
    ...
    if (!player) {
      Donkeycraft.Logger.error(
        'Movement',
        'Player reference is null in tick()'
      );
      return;
    }
    if (!this._input) {
    ...
    if (!this._collision) {
      Donkeycraft.Logger.warn(
        'Movement',
        'Collision reference is null — movement will be unconstrained'
      );
    }
  ...
  Donkeycraft.Movement.prototype._tickSurvival = function (deltaTime) {
    ...
    if (!player || !input || !collision) {
      if (!player)
        Donkeycraft.Logger.error(
          'Movement',
          'Player reference is null in _tickSurvival()'
        );
      if (!input)
        Donkeycraft.Logger.error(
          'Movement',
          'Input reference is null in _tickSurvival()'
        );
      if (!collision)
        Donkeycraft.Logger.warn(
          'Movement',
    ...
    var forward = input.isKeyDown(Config.KEYBINDS.MOVE_FORWARD)
      ? 1
      : input.isKeyDown(Config.KEYBINDS.MOVE_BACKWARD)
        ? -1
        : 0;
    var strafe = this._mouseLocked
      ? input.isKeyDown(Config.KEYBINDS.MOVE_RIGHT)
        ? 1
    ...
    var eyesInWater = this._collision.isBlockLiquid(
      pos.x,
      Math.floor(pos.y + Config.PLAYER_EYE_HEIGHT),
      pos.z
    );
    ...
    if (eyesInWater) {
      ...
      var swimUp = input.isKeyDown(Config.KEYBINDS.JUMP)
        ? 1
        : input.isKeyDown(Config.KEYBINDS.SPEED_UP)
          ? 1
          : 0;
    ...
    if (result.onGround) {
      if (anyInLava) {
        ...
        if (
          player.maxFallDistance > Config.FALL_DAMAGE_THRESHOLD &&
          this._playerRef
        ) {
          try {
            ...
            if (fallDmg > 0) {
              Donkeycraft.Logger.info(
                'Movement',
                'Player took ' + fallDmg.toFixed(1) + ' fall damage'
              );
            }
          } catch (e) {
            Donkeycraft.Logger.warn(
              'Movement',
              'Fall damage application failed: ' + e.message
            );
          }
        } else {
        ...
        if (
          ...
          try {
            ...
            if (fallDmg2 > 0) {
              Donkeycraft.Logger.info(
                'Movement',
                'Player took ' + fallDmg2.toFixed(1) + ' fall damage'
              );
            }
          } catch (e) {
            Donkeycraft.Logger.warn(
    ...
    if (terrainContext.type && terrainContext.slowModifier < 1.0) {
      Donkeycraft.Logger.info(
        ...
        'Terrain slow effect: ' +
          terrainContext.type +
          ' (' +
          terrainContext.slowModifier * 100 +
          '% speed)'
      );
    }
  };
  ...
  Donkeycraft.Movement.prototype._tickCreativeFly = function (deltaTime) {
    ...
    if (!player || !input || !collision) {
      if (!player)
        Donkeycraft.Logger.error(
          'Movement',
          'Player reference is null in _tickCreativeFly()'
        );
      if (!input)
        Donkeycraft.Logger.error(
          'Movement',
          'Input reference is null in _tickCreativeFly()'
        );
      if (!collision)
        Donkeycraft.Logger.warn(
          'Movement',
    ...
    var forward = input.isKeyDown(Config.KEYBINDS.MOVE_FORWARD)
      ...
      : input.isKeyDown(Config.KEYBINDS.MOVE_BACKWARD)
        ? -1
        : 0;
    var strafe = input.isKeyDown(Config.KEYBINDS.MOVE_RIGHT)
      ? 1
      : input.isKeyDown(Config.KEYBINDS.MOVE_LEFT)
        ? -1
        : 0;
    ...
    var moveUp = input.isKeyDown(Config.KEYBINDS.JUMP)
      ? 1
      : input.isKeyDown(Config.KEYBINDS.SPEED_UP)
        ? 1
        : 0;
  ...
  Donkeycraft.Movement.prototype.getHorizontalSpeed = function () {
    ...
    return this._getSpeedForIndex(this.getSliderIndex(), gameMode, {
      slowModifier: 1.0,
    });
  };
   ...
   * @private
   */
  Donkeycraft.Movement.prototype._isPlayerInLiquid = function (
    pos,
    dimensions
  ) {
    var collision = this._collision;
