...
(function () {
  ...
  Donkeycraft.EndGenerator = (function () {
    ...
    /**
     * Internal cache for end terrain heightmaps keyed by chunk position.
     * Format: "{chunkX},{chunkZ}" -> heightmap array.
     ...
     * Uses cached heightmap for quick terrain queries to avoid redundant noise computation.
    ...
    function generateEndTerrain(chunkOrX, optChunkX, optChunkZ) {
      ...
      // Cache and mark heightmap for this chunk
      var hm = generateEndHeightmap(cx, cz);
      _heightmapCache[cx + ',' + cz] = hm;
    ...
    function _getIslandType(chunkX, chunkZ) {
      ...
      if (dist < 12) {
        var noise = Donkeycraft.PerlinNoise.noise2D(chunkX * 0.1, chunkZ * 0.1);
    ...
    function _generateEndCity(chunk, chunkX, chunkZ) {
      ...
      for (var p = 0; p < pillarHeights.length; p++) {
        ...
        for (var py = 0; py < ph; py++) {
          ...
          // Break early if pillar extends beyond world height to avoid wasted iterations.
          if (pillarY >= WORLD_HEIGHT - 5 || pillarY < 0) break;
    ...
    /**
     * Clear the heightmap cache. Call during dimension reload or world reset.
     */
    ...
    /**
     * Get a cached heightmap for a chunk position, if available.
     * @param {number} chunkX - Chunk X coordinate.
     * @param {number} chunkZ - Chunk Z coordinate.
     * @returns {number[]|null} Cached heightmap array, or null if not cached.
     */
    function getCachedHeightmap(chunkX, chunkZ) {
      return _heightmapCache[chunkX + ',' + chunkZ] || null;
    }

    /**
     * Destroy and free resources.
     * Clears all cached block IDs, heightmaps, and chunk manager reference.
     */
    function destroy() {
      _chunkManager = null;
      _endStoneId = 0;
    ...
    return {
      ...
      getCachedHeightmap: getCachedHeightmap,
      clearHeightmapCache: clearHeightmapCache,
