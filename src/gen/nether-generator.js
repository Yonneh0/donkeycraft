...
(function () {
    ...
    var _oreVeinConfig = {
        quartz: { veinsPerChunk: 20, veinSize: 8, minY: 5, maxY: WORLD_HEIGHT - 5, rarity: 0.15 },
        gold: { veinsPerChunk: 16, veinSize: 7, minY: 5, maxY: WORLD_HEIGHT - 5, rarity: 0.10 },
        gilded: { veinsPerChunk: 12, veinSize: 6, minY: 5, maxY: WORLD_HEIGHT - 5, rarity: 0.08 },
        magma: { veinsPerChunk: 8, veinSize: 5, minY: 5, maxY: WORLD_HEIGHT - 5, rarity: 0.05 },
        ancientDebris: { veinsPerChunk: 1, veinSize: 4, minY: 8, maxY: 22, rarity: 0.01 }
    };
    ...
    Donkeycraft.NetherGenerator = (function () {
            ...
            for (var x = 0; x < CHUNK_SIZE; x++) {
                for (var z = 0; z < CHUNK_SIZE; z++) {
                    ...
                    var lavaSurfaceNoise = Donkeycraft.PerlinNoise.fbm(
                        worldX * 0.025, 0, worldZ * 0.025, 3, 0.5, 2.0
                    );

                    // Map noise to height offset: fbm returns [-1, 1], clamp to [-2, +2]
                    var heightOffset = Math.max(-2, Math.min(2, Math.floor(lavaSurfaceNoise * 2.5)));
                    ...
                    for (var y = minLavaY; y <= localLavaY && y >= 0 && y < WORLD_HEIGHT; y++) {
                        chunk.setBlock(x, y, z, _lavaId);
                    }

                    // Add one extra block above surface for pooling effect (only where noise peaks)
                    if (lavaSurfaceNoise > 0.4 && localLavaY + 1 >= 0 && localLavaY + 1 < WORLD_HEIGHT) {
        ...
        function _generateOreVeins(chunk, chunkX, chunkZ) {
            ...
            var oreTypes = [
                ...
                { id: _gildedBlackstoneId, config: _oreVeinConfig.gilded, noiseOffset: 500 },
                { id: _magmaBlockId, config: _oreVeinConfig.magma, noiseOffset: 600 },
                { id: _ancientDebrisId, config: _oreVeinConfig.ancientDebris, noiseOffset: 700 }
            ];
            ...
            for (var o = 0; o < oreTypes.length; o++) {
                ...
                for (var v = 0; v < config.veinsPerChunk; v++) {
                    ...
                    var veinZ = ((veinHash >> 8) % CHUNK_SIZE + CHUNK_SIZE) % CHUNK_SIZE;
                    ...
                    _placeOreVein(chunk, veinX, veinY, veinZ, config.veinSize, oreType.id, config.rarity);
                }
            }
        }
