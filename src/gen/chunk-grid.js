...
(function () {
  ...
  function setRadii(radii) {
    if (radii.n !== undefined)
      _radiusN = Math.max(
        MIN_CHUNK_RADIUS,
        Math.min(MAX_CHUNK_RADIUS, Math.round(radii.n))
      );
    if (radii.s !== undefined)
      _radiusS = Math.max(
        MIN_CHUNK_RADIUS,
        Math.min(MAX_CHUNK_RADIUS, Math.round(radii.s))
      );
    if (radii.e !== undefined)
      _radiusE = Math.max(
        MIN_CHUNK_RADIUS,
        Math.min(MAX_CHUNK_RADIUS, Math.round(radii.e))
      );
    if (radii.w !== undefined)
      _radiusW = Math.max(
        MIN_CHUNK_RADIUS,
        Math.min(MAX_CHUNK_RADIUS, Math.round(radii.w))
      );
  ...
  function resetToDefault() {
    ...
    _radiusS =
      DEFAULT_GRID_DIMENSION - Math.floor(DEFAULT_GRID_DIMENSION / 2) - 1;
  ...
  function loadState(key, store) {
    ...
    try {
      ...
      var loadedRadii = {
        n: Math.max(
          MIN_CHUNK_RADIUS,
          Math.min(MAX_CHUNK_RADIUS, Math.round(state.radii.n || 0))
        ),
        s: Math.max(
          MIN_CHUNK_RADIUS,
          Math.min(MAX_CHUNK_RADIUS, Math.round(state.radii.s || 0))
        ),
        e: Math.max(
          MIN_CHUNK_RADIUS,
          Math.min(MAX_CHUNK_RADIUS, Math.round(state.radii.e || 0))
        ),
        w: Math.max(
          MIN_CHUNK_RADIUS,
          Math.min(MAX_CHUNK_RADIUS, Math.round(state.radii.w || 0))
        ),
      };
