...
(function () {
  ...
  function generateHeightmap(chunkX, chunkZ, biomeId) {
    ...
    var validParams =
      isFinite(params.continentalScale) &&
      isFinite(params.terrainScale) &&
      isFinite(params.detailScale) &&
      isFinite(params.ridgeScale) &&
      isFinite(params.microScale) &&
      isFinite(params.baseHeight) &&
      isFinite(params.heightVariation) &&
      params.heightVariation >= 0;
    if (!validParams) {
      return _createEmptyHeightmap();
    }
    ...
    for (var x = 0; x < CHUNK_SIZE; x++) {
      for (var z = 0; z < CHUNK_SIZE; z++) {
        ...
        var continentalNoise = _safeFbm(
          worldX * params.continentalScale,
          0,
          worldZ * params.continentalScale,
          params.continentalOctaves
        );

        // Pass 2: Terrain shaping — hills/valleys
        var terrainNoise = _safeFbm(
          worldX * params.terrainScale,
          0,
          worldZ * params.terrainScale,
          params.terrainOctaves
        );

        // Pass 3: Detail noise — texture and variation
        var detailNoise = _safeFbm(
          worldX * params.detailScale,
          0,
          worldZ * params.detailScale,
          params.detailOctaves
        );

        // Pass 4: Ridged multifractal — mountains/cliffs
        var ridgeNoise = _safeRidgedMultifractal(
          worldX * params.ridgeScale,
          0,
          worldZ * params.ridgeScale,
          params.ridgeOctaves,
          params.ridgeAmplitude,
          params.ridgeLacunarity,
          params.ridgeGain
        );

        // Pass 5: Micro-detail — surface roughness
        var microNoise = _safeFbm(
          worldX * params.microScale,
          0,
          worldZ * params.microScale,
          params.microOctaves
        );
        ...
        var valleyStrength = _safeValleyNoise(
          ...
          0,
          worldZ * params.continentalScale * 1.5,
          3
        );
   ...
   * @private
   */
  function _safeRidgedMultifractal(
    x,
    y,
    z,
    octaves,
    ...
    try {
      if (
        Donkeycraft._gen &&
        typeof Donkeycraft._gen._ridgedMultifractal === 'function'
      ) {
        var val = Donkeycraft._gen._ridgedMultifractal(
          x,
          y,
          z,
          octaves || 3,
  ...
  function _safeValleyNoise(x, y, z, octaves) {
    try {
      if (
        Donkeycraft._gen &&
        typeof Donkeycraft._gen._valleyNoise === 'function'
      ) {
   ...
   * @private
   */
  function _calculateShoreDistance(
    continentalNoise,
    terrainNoise,
    detailNoise,
    ridgeNoise,
  ...
  function getBiomeParameters(biomeName) {
    ...
    if (typeof console !== 'undefined' && console.warn) {
      console.warn(
        '[SurfaceGenerator] Biome "' +
          biomeName +
          '" has invalid parameters, skipping'
      );
    }
    return null;
  }
  ...
  function _ridgedMultifractal(x, y, z, octaves, amplitude, frequency, gain) {
    return _safeRidgedMultifractal(
      x,
      y,
      z,
      octaves,
