...
(function () {
  ...
  function generateGrassTop() {
    ...
    return _cacheTexture
      ? _cacheTexture('grass_top', 'default', result)
      : result;
  }
  ...
  function generateGrassSide() {
    ...
    var result = _canvasToImage(canvas);
    return _cacheTexture
      ? _cacheTexture('grass_side', 'default', result)
      : result;
  }
    ...
    for (var y = 0; y < TEX_SIZE; y++) {
      for (var x = 0; x < TEX_SIZE; x++) {
        ...
        imgData.data[idx + 1] = Math.max(
          0,
          Math.min(255, 195 + n + layerEffect)
        );
        imgData.data[idx + 2] = Math.max(
          0,
          Math.min(255, 150 + n + layerEffect)
        );
        imgData.data[idx + 3] = 255;
      }
    }
  ...
  Donkeycraft.TextureGenerator.generateChiseledSandstone =
    generateChiseledSandstone;
  ...
  Donkeycraft.TextureGenerator.generatePolishedGranite =
    generatePolishedGranite;
  Donkeycraft.TextureGenerator.generatePolishedDiorite =
    generatePolishedDiorite;
  Donkeycraft.TextureGenerator.generatePolishedAndesite =
    generatePolishedAndesite;

  // Aliases for cross-module use
  /** @type {Function} */
  Donkeycraft.TextureGenerator.generateRedSand = function () {
    return generateSand(183, 105, 63);
  };
  Donkeycraft.TextureGenerator.generateSnowBlock = generateSnowBlock;
  /** @type {Function} */
  Donkeycraft.TextureGenerator.generateGrass = function () {
