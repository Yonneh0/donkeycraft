...
(function () {
  ...
  function _placeRivers(chunk, chunkX, chunkZ, config, heightmap) {
    ...
    var startX =
      (((chunkX * 7 + chunkZ * 13) % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE;
    var startZ =
      (((chunkX * 11 + chunkZ * 3) % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE;
    ...
    for (var step = 0; step < riverLength; step++) {
      ...
      if (
        localX < -riverWidth * 2 ||
        localX >= CHUNK_SIZE + riverWidth * 2 ||
        localZ < -riverWidth * 2 ||
        localZ >= CHUNK_SIZE + riverWidth * 2
      ) {
        break;
      }
      ...
      if (
        sampleLocalX < 0 ||
        sampleLocalX >= CHUNK_SIZE ||
        sampleLocalZ < 0 ||
        sampleLocalZ >= CHUNK_SIZE
      ) {
        break;
      }

      currentY =
        heightmap[sampleLocalX + sampleLocalZ * CHUNK_SIZE] ||
        config.waterLevel;
      ...
      for (var dy = 0; dy <= RIVER_DEPTH && currentY - dy >= 0; dy++) {
        ...
        for (var wx = -riverWidth; wx <= riverWidth; wx++) {
          for (var wz = -riverWidth; wz <= riverWidth; wz++) {
            ...
            var normalizedDist =
              (wx * wx + wz * wz) / ((riverWidth + 1) * (riverWidth + 1));
      ...
      for (
        var wy = currentY - RIVER_DEPTH;
        wy <= currentY && wy < WORLD_HEIGHT;
        wy++
      ) {
      ...
      for (var nx = -1; nx <= 1; nx++) {
        for (var nz = -1; nz <= 1; nz++) {
          ...
          var neighborLocalX =
            ((neighborWorldX % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE;
          var neighborLocalZ =
            ((neighborWorldZ % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE;
          ...
          var neighborHeight =
            heightmap[neighborLocalX + neighborLocalZ * CHUNK_SIZE];
  ...
  function _fbmNoise(x, y, z, octaves) {
    if (Donkeycraft._gen && typeof Donkeycraft._gen._fbm === 'function') {
      try {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            '[WaterGenerator] FBM noise fallback:',
    ...
    if (Donkeycraft._gen && typeof Donkeycraft._gen._noise2D === 'function') {
      try {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            '[WaterGenerator] Noise2D fallback:',
            e && e.message ? e.message : String(e)
          );
        }
      }
