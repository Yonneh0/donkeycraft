...
(function () {
  ...
  Donkeycraft.MobSpawnDefinition = function (config) {
     ...
     * @type {boolean}
     */
    this.requireSolidBelow =
      config.requireSolidBelow !== undefined ? config.requireSolidBelow : true;
  };
    ...
    if (
      ...
      if (Donkeycraft.Logger) {
        Donkeycraft.Logger.warn(
          'MobSpawner',
          'registerSpawn called with invalid definition: ' +
   ...
   */
  Donkeycraft.MobSpawner.prototype.isValidSpawnChunk = function (
    chunkX,
    chunkZ,
    getChunkInfo
  ) {
   ...
   */
  Donkeycraft.MobSpawner.prototype.validateSpawnPosition = function (
    definition,
    x,
    y,
    z,
    ...
    if (
      typeof getBlockLight !== 'function' ||
      typeof isBlockSolid !== 'function'
    ) {
      return false;
    }
    ...
    if (
      ...
      lightLevel < definition.minLightLevel ||
      lightLevel > definition.maxLightLevel
    ) {
      return false;
    }
   ...
   */
  Donkeycraft.MobSpawner.prototype.findSpawnPosition = function (
    definition,
    chunkX,
    chunkZ,
    getBlockLight,
   ...
   */
  Donkeycraft.MobSpawner.prototype.spawnMobAt = function (
    definition,
    x,
    y,
    z,
    ...
    this._currentCounts[definition.type] =
      (this._currentCounts[definition.type] || 0) + 1;
  ...
  Donkeycraft.MobSpawner.prototype.tick = function (deltaTime, worldInfo) {
    ...
    var intervalSeconds =
      this.spawnCheckInterval / (Config.GAME_TICKS_PER_SECOND || 20);
    ...
    for (var d = 0; d < this._definitions.length; d++) {
      ...
      for (var a = 0; a < attempts; a++) {
        ...
        if (
          worldInfo.getCurrentBiome &&
          typeof worldInfo.getCurrentBiome === 'function'
        ) {
          currentBiome = worldInfo.getCurrentBiome(
            chunkX * 16 + 7,
            chunkZ * 16 + 7
          );
        }
        ...
        if (pos) {
          ...
          var groupSize =
            definition.minGroupSize +
            Math.floor(
              Math.random() *
                (definition.maxGroupSize - definition.minGroupSize + 1)
            );
  ...
  Donkeycraft.MobSpawner.defaultDefinitions = function () {
    ...
    /** @type {Donkeycraft.MobSpawnDefinition} */
    defs.push(
      new Donkeycraft.MobSpawnDefinition({
        type: 'zombie',
