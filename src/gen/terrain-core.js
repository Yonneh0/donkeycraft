...
(function () {
  ...
  function setSeed(seed) {
    ...
    if (
      Donkeycraft.PerlinNoise &&
      typeof Donkeycraft.PerlinNoise.init === 'function'
    ) {
      try {
  ...
  function setBiome(biome) {
    ...
    if (typeof biome === 'object' && biome.id !== undefined) {
      ...
      if (!isFinite(biome)) {
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            '[TerrainCore] setBiome: non-finite biome ID, using default ID 0 (grass)'
          );
        }
      } else {
    ...
    if (!biomeResolved && typeof console !== 'undefined' && console.warn) {
      console.warn(
        '[TerrainCore] setBiome: unknown biome "' +
          String(biome) +
          '", defaulting to ID 0 (grass)'
      );
    }
  ...
  function generateChunk(chunkX, chunkZ) {
    ...
    if (storageReady) {
      return Donkeycraft.Storage.getChunk(cacheKey)
        .then(function (cachedData) {
          // Check if cached data exists and has a valid heightmap
          if (
            cachedData &&
            Array.isArray(cachedData.heightmap) &&
            cachedData.heightmap.length > 0
          ) {
        ...
        .catch(function (e) {
          ...
          if (typeof console !== 'undefined' && console.warn) {
            console.warn(
              '[TerrainCore] Storage error for chunk (' +
                chunkX +
                ',' +
                chunkZ +
                '):',
  ...
  function _generateChunkData(chunkX, chunkZ) {
    ...
    try {
      ...
      var heightmap = Donkeycraft.SurfaceGenerator.generateHeightmap(
        chunkX,
        chunkZ,
        biomeName
      );
      ...
      if (typeof console !== 'undefined' && console.warn) {
        console.warn(
          '[TerrainCore] Generation error for chunk (' +
            chunkX +
            ',' +
            chunkZ +
            '):',
  ...
  function generateAllChunks() {
    ...
    if (
      ...
      typeof Donkeycraft.ChunkGrid.getGridChunks === 'function'
    ) {
      try {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            '[TerrainCore] ChunkGrid.getGridChunks failed:',
            e && e.message ? e.message : String(e)
          );
        }
      }
    ...
    for (var i = 0; i < gridChunks.length; i++) {
      (function (chunkX, chunkZ) {
        promises.push(
          generateChunk(chunkX, chunkZ).catch(function (e) {
            if (typeof console !== 'undefined' && console.warn) {
              console.warn(
                '[TerrainCore] Failed to generate chunk (' +
                  chunkX +
                  ',' +
                  chunkZ +
                  '):',
    ...
    return Promise.allSettled(promises)
      .then(function (results) {
        ...
        for (var i = 0; i < results.length; i++) {
          if (results[i].status === 'fulfilled') {
            ...
            if (typeof console !== 'undefined' && console.warn) {
              console.warn(
                '[TerrainCore] Chunk generation rejected:',
      ...
      .catch(function (error) {
        ...
        if (typeof console !== 'undefined' && console.error) {
          console.error(
            '[TerrainCore] generateAllChunks failed:',
  ...
  function getStats() {
    ...
    var cacheHitRate =
      totalAccesses > 0 ? (_cacheHitCount / totalAccesses) * 100 : 0;
  ...
  function init() {
    if (Donkeycraft.Storage && typeof Donkeycraft.Storage.init === 'function') {
      return Donkeycraft.Storage.init()
        .then(function (ready) {
          setSeed(_seed);
          return true;
        })
        .catch(function (e) {
          if (typeof console !== 'undefined' && console.warn) {
            console.warn(
              '[TerrainCore] Storage initialization failed:',
              e && e.message ? e.message : String(e)
            );
          }
          setSeed(_seed);
  ...
  function destroy() {
    if (
      Donkeycraft.Storage &&
      typeof Donkeycraft.Storage.destroy === 'function'
    ) {
