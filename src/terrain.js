...
(function () {
   ...
   */
  Donkeycraft.TerrainController.prototype.registerKeyAction = function (
    keyCode,
    callback
  ) {
   ...
   */
  Donkeycraft.TerrainController.prototype.unregisterKeyAction = function (
    keyCode
  ) {
   ...
   */
  Donkeycraft.TerrainController.prototype.setCameraPosition = function (
    x,
    y,
    z
  ) {
   ...
   */
  Donkeycraft.TerrainController.prototype.setCameraRotation = function (
    yaw,
    pitch
  ) {
  ...
  Donkeycraft.TerrainController.prototype.updateCamera = function (dt) {
    ...
    var sens =
      Donkeycraft.Config && Donkeycraft.Config.MOUSE_SENSITIVITY
        ? Donkeycraft.Config.MOUSE_SENSITIVITY
        : 0.15;
    ...
    this._camera.pitch = Math.max(
      -Math.PI / 2 + 0.01,
      Math.min(Math.PI / 2 - 0.01, this._camera.pitch)
    );
    ...
    var keys = this._keys;
    if (keys['KeyW']) {
      this._camera.x += fwd[0] * spd;
      this._camera.z += fwd[2] * spd;
    }
    if (keys['KeyS']) {
      this._camera.x -= fwd[0] * spd;
      this._camera.z -= fwd[2] * spd;
    ...
    if (keys['KeyA']) {
      this._camera.x -= rgt[0] * spd;
      this._camera.z -= rgt[2] * spd;
    }
    if (keys['KeyD']) {
      this._camera.x += rgt[0] * spd;
      this._camera.z += rgt[2] * spd;
   ...
   */
  Donkeycraft.TerrainController.prototype.updateFps = function (
    ts,
    fpsElementIds
  ) {
    // FPS tracking
  ...
  Donkeycraft.TerrainController.prototype.placeAboveGround = function () {
    ...
    for (var y = ws - 1; y >= 0; y--) {
      if (localX >= 0 && localX < cs && localZ >= 0 && localZ < cs) {
        ...
        var chunk = window._dkTerrainRenderer
          ? window._dkTerrainRenderer._chunks.get(key)
          : null;
        if (chunk) {
   ...
   */
  Donkeycraft.TerrainController.prototype.setOverviewView = function (
    centerChunkX,
    centerChunkZ,
    radiusN,
    radiusS,
    ...
    this._camera.yaw = Math.atan2(
      -(targetX - this._camera.x),
      -(targetZ - this._camera.z)
    );
  ...
  Donkeycraft.TerrainController.prototype.saveState = function () {
    try {
      ...
      localStorage.setItem(
        'donkeycraft_terrain_controller_state',
  ...
  Donkeycraft.TerrainController.prototype.loadState = function () {
    try {
      ...
      if (state.camera && typeof state.camera === 'object') {
        ...
        if (typeof state.camera.yaw === 'number')
          this._camera.yaw = state.camera.yaw;
        if (typeof state.camera.pitch === 'number')
          this._camera.pitch = state.camera.pitch;
      }
      if (typeof state.speedLevel === 'number')
        this._speedLevel = state.speedLevel;
   ...
   */
  Donkeycraft.TerrainController.prototype.mat4Perspective = function (
    o,
    fov,
    aspect,
    near,
   ...
   */
  Donkeycraft.TerrainController.prototype.mat4LookAt = function (
    o,
    eye,
    center,
    up
   ...
   */
  Donkeycraft.TerrainController.prototype.mat4Translate = function (
    o,
    tx,
    ty,
    tz
