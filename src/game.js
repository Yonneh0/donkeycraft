...
(function () {
  ...
  Donkeycraft.Game.prototype.init = function () {
    try {
      ...
      if (Donkeycraft.WaterRenderer) {
        try {
          this._waterRenderer = new Donkeycraft.WaterRenderer(
            this._gl,
            this._shaderManager
          );
          ...
          var waterLevel = 63;
          if (
            Donkeycraft.TerrainGenerator &&
            Donkeycraft.TerrainGenerator.getWaterLevel !== undefined
          ) {
          ...
          Donkeycraft.Logger.error(
            'Game',
            'WaterRenderer initialization failed: ' + e.message
          );
        }
      }

      // Create hand renderer
      this._handRenderer = new Donkeycraft.HandRenderer(
        this._gl,
        this._shaderManager
      );

      // Create break particles system
      this._breakParticles = new Donkeycraft.BreakParticles(
        this._gl,
        this._shaderManager
      );

      // Create GUI renderer
      this._guiRenderer = new Donkeycraft.GUIRenderer(
        this._gl,
        this._shaderManager
      );
      ...
      if (Donkeycraft.WeatherRenderer) {
        this._weatherRenderer = new Donkeycraft.WeatherRenderer(
          this._gl,
          this._shaderManager
        );
      }
      ...
      if (Donkeycraft.TerrainCore) {
        try {
          ...
          Donkeycraft.Logger.info(
            'Game',
            'TerrainCore initialized — unified caching enabled'
          );
        } catch (e) {
          Donkeycraft.Logger.warn(
            'Game',
            'TerrainCore init failed: ' +
      ...
      if (Donkeycraft.ChunkGrid) {
        try {
          ...
          this._chunkGrid.onGridChange(function (event) {
            if (
              event.type === 'gridExpanded' ||
              event.type === 'gridContracted' ||
              event.type === 'gridResized'
            ) {
              // Update chunk manager render distance when grid changes
              if (
                gameForGrid._chunkManager &&
                gameForGrid._chunkManager.setRenderDistance
              ) {
                ...
                gameForGrid._chunkManager.setRenderDistance(
                  Math.max(2, Math.min(16, radius * 2 + 2))
                );
              }
            }
          });
          Donkeycraft.Logger.info(
            'Game',
            'ChunkGrid wired — chunk management coordinated'
          );
        } catch (e) {
          Donkeycraft.Logger.warn(
            'Game',
            'ChunkGrid init failed: ' + (e && e.message ? e.message : String(e))
          );
        }
      }
      ...
      cm.onChunksChanged = function () {
        ...
        for (var _i = 0; _i < dirty.length; _i++) {
          self._terrainRenderer.markChunkDirty(
            dirty[_i].chunk.chunkX,
            dirty[_i].chunk.chunkZ
          );
        }
      };
      ...
      try {
        ...
        if (
          ...
          typeof Donkeycraft.SurfaceGenerator.init === 'function'
        ) {
        ...
        if (
          ...
          Donkeycraft.Logger.info(
            'Game',
            'TerrainSurface initialized — surface layers enabled'
          );
        } else if (Donkeycraft.TerrainSurface) {
          Donkeycraft.Logger.warn(
            'Game',
            'TerrainSurface.init() not available — surface layers may not work'
          );
        }
        // WaterGenerator — lake detection, surface water flow
        if (
          Donkeycraft.WaterGenerator &&
          typeof Donkeycraft.WaterGenerator.init === 'function'
        ) {
        ...
        if (
          ...
          typeof Donkeycraft.StructureGenerator.init === 'function'
        ) {
      ...
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Initial chunk generation had errors: ' + e.message
        );
      }
      ...
      if (spawnPos && spawnPos.y > 0) {
        ...
        this._camera.setPosition(
          spawnPos.x,
          spawnPos.y + Config.PLAYER_EYE_HEIGHT,
          spawnPos.z
        );
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'No land found by exhaustive search — using fallback spawn'
        );
        var fallbackY = Config.WORLD_HEIGHT - 10;
        var fallbackChunk = this._chunkManager.getChunk(
          initSpawnCX,
          initSpawnCZ
        );
        if (fallbackChunk) {
          for (var fy = Config.WORLD_HEIGHT - 1; fy >= 0; fy--) {
            ...
            if (
              ...
              Donkeycraft.BlockTypes.isSolid(fb)
            ) {
              fallbackY = fy + 2;
              break;
            }
        ...
        this._camera.setPosition(
          0.5,
          fallbackY + Config.PLAYER_EYE_HEIGHT,
          0.5
        );
      }
      ...
      if (Donkeycraft.Portal && Donkeycraft.Portal.init) {
        var currentDim = Donkeycraft.Dimensions
          ? Donkeycraft.Dimensions.getCurrentDimension()
          : 0;
        var dimChunkManager =
          Donkeycraft.Dimensions.getChunkManagerForDimension(currentDim);
        Donkeycraft.Portal.init(
          this._eventBus,
          dimChunkManager || this._chunkManager,
          currentDim
        );
      }
      ...
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'KeybindingsPanel init failed: ' + e.message
        );
      }
  ...
  Donkeycraft.Game.prototype.setLevelData = function (levelData, worldName) {
    ...
    if (levelData && this._worldStore && worldName) {
      try {
        var intervalMs =
          Donkeycraft.Config && Donkeycraft.Config.LEVEL_DATA_AUTO_SAVE_INTERVAL
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to start LevelData auto-save: ' + e.message
        );
      }
    }
  };
   ...
   */
  Donkeycraft.Game.prototype.setEntitySystems = function (
    entityEngine,
    entityManager,
    entityRenderer
  ) {
    ...
    if (Donkeycraft.DebugTerrainGenerator) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire DebugTerrainGenerator references: ' + e.message
        );
      }
    }
    ...
    if (this._entityEngine && this._timer) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire timer to entity engine: ' + e.message
        );
      }
    }
    ...
    if (this._entityRenderer && this._gl && this._shaderManager) {
      try {
        ...
        if (
          typeof this._entityRenderer.validateShaderCompatibility === 'function'
        ) {
          try {
            ...
            if (!validation.valid) {
              Donkeycraft.Logger.warn(
                'Game',
                'Entity renderer shader compatibility check failed:'
              );
              ...
              Donkeycraft.Logger.debug(
                'Game',
                'Entity renderer shader compatibility validated successfully.'
              );
            }
          } catch (e) {
            Donkeycraft.Logger.warn(
              'Game',
              'Entity renderer shader validation threw exception: ' + e.message
            );
          }
        }
      } catch (e) {
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire WebGL to entity renderer: ' + e.message
        );
      }
    }
    ...
    if (this._entityRenderer && this._entityEngine) {
      try {
        ...
        if (engineEntityManager) {
          ...
          Donkeycraft.Logger.warn(
            'Game',
            'EntityEngine.getEntityManager() returned null — renderer will not see any entities.'
          );
        }
      } catch (e) {
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire EntityManager to entity renderer: ' + e.message
        );
      }
    }
    ...
    if (this._entityRenderer && this._camera) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire camera to entity renderer: ' + e.message
        );
      }
    }
    ...
    if (this._entityEngine && this._entityRenderer) {
      try {
        this._entityRenderer.setRenderDistance(
          this._entityEngine.renderDistance || 16
        );
      } catch (e) {
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to sync render distance: ' + e.message
        );
      }
    }
    ...
    if (this._entityManager) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire block query to entity manager: ' + e.message
        );
      }
    }
    ...
    if (this._entityManager && this._player) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Failed to wire player entity: ' + e.message
        );
      }
    }
  };
  ...
  Donkeycraft.Game.prototype._getRendererMap = function () {
    return {
      sky: '_renderSky',
      terrain: '_renderTerrain',
      water: '_renderWater',
      particles: '_renderParticles',
      hand: '_renderHand',
      weather: '_renderWeather',
      gui: '_renderGUI',
  ...
  Donkeycraft.Game.prototype.setSystems = function (
    ...
    if (!this._chunkManager) {
      Donkeycraft.Logger.warn(
        'Game',
        'setSystems called before init: chunkManager not available'
      );
      return false;
    }
    if (!this._player) {
      Donkeycraft.Logger.warn(
        'Game',
        'setSystems called before init: player not available'
      );
      return false;
    }
    if (!this._input) {
      Donkeycraft.Logger.warn(
        'Game',
        'setSystems called before init: input not available'
      );
      return false;
    }
    ...
    if (collisionSystem && typeof collisionSystem === 'function') {
      try {
        ...
        Donkeycraft.Logger.error(
          'Game',
          'Collision instantiation failed: ' + e.message
        );
        this._collisionSystem = null;
      }
    ...
    if (movementSystem && typeof movementSystem === 'function') {
      try {
        ...
        Donkeycraft.Logger.error(
          'Game',
          'Movement instantiation failed: ' + e.message
        );
        this._movementSystem = null;
      }
    ...
    if (jumpSystem && typeof jumpSystem === 'function') {
      try {
        this._jumpSystem = new jumpSystem(this._player, this._input);
      } catch (e) {
        Donkeycraft.Logger.error(
          'Game',
          'Jump instantiation failed: ' + e.message
        );
        this._jumpSystem = null;
      }
    } else if (jumpSystem) {
    ...
    if (flyingSystem && typeof flyingSystem === 'function') {
      try {
        this._flyingSystem = new flyingSystem(this._player, this._input);
      } catch (e) {
        Donkeycraft.Logger.error(
          'Game',
          'Flying instantiation failed: ' + e.message
        );
        this._flyingSystem = null;
      }
    ...
    if (hungerConstructor && typeof hungerConstructor === 'function') {
      try {
        ...
        Donkeycraft.Logger.error(
          'Game',
          'Hunger instantiation failed: ' + e.message
        );
        this._hungerSystem = null;
      }
    ...
    if (this._redstoneEngine) {
      ...
      if (Donkeycraft.RedstoneRepeaterComparator) {
        this._redstoneEngine.setRepeaterComparator(
          Donkeycraft.RedstoneRepeaterComparator
        );
      }
  ...
  Donkeycraft.Game.prototype.destroy = function () {
    ...
    if (this._redstoneEngine) {
      if (typeof this._redstoneEngine.destroy === 'function') {
        try {
          ...
          Donkeycraft.Logger.warn(
            'Game',
            'Redstone engine destroy error: ' + e.message
          );
        }
      }
      this._redstoneEngine = null;
    ...
    if (this._eventBus) {
      if (typeof this._eventBus.clear === 'function') {
        try {
          ...
          Donkeycraft.Logger.warn('Game', 'EventBus clear error: ' + e.message);
        }
      }
      this._eventBus = null;
    }
    ...
    if (this._keybindingsPanel) {
      try {
        this._keybindingsPanel.destroy();
      } catch (e) {
        Donkeycraft.Logger.warn(
          'Game',
          'KeybindingsPanel destroy error: ' + e.message
        );
      }
      this._keybindingsPanel = null;
    }
      ...
      try {
        this._levelData
          ...
          .catch(function () {
            Donkeycraft.Logger.warn(
              'Game',
              'Final world save failed on destroy — destroying WorldStore anyway'
            );
            if (self._worldStore && self._worldStore.destroy) {
              try {
                self._worldStore.destroy();
    ...
    if (this._todUI) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'TimeOfDayUI destroy error: ' + e.message
        );
      }
      this._todUI = null;
    }
    ...
    if (this._terrainCore && typeof this._terrainCore.destroy === 'function') {
      try {
        this._terrainCore.destroy();
      } catch (e) {
        Donkeycraft.Logger.warn(
          'Game',
          'TerrainCore destroy error: ' + e.message
        );
      }
      this._terrainCore = null;
    }
    ...
    if (
      this._chunkGrid &&
      typeof this._chunkGrid.getListenerCount === 'function'
    ) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'ChunkGrid cleanup error: ' + e.message
        );
      }
      this._chunkGrid = null;
    }
  ...
  Donkeycraft.Game.prototype._buildTextureAtlasAsync = function () {
    if (!this._gl || !Donkeycraft.AssetManager || !Donkeycraft.BlockRegistry) {
      Donkeycraft.Logger.warn(
        'Game',
        'Texture atlas build skipped — required systems not available'
      );
      return;
    }
    ...
    if (!generatedTex || Object.keys(generatedTex).length === 0) {
      Donkeycraft.Logger.warn(
        'Game',
        'No generated textures found in AssetManager'
      );
      return;
    }
      ...
      if (processed < totalBlocks) {
        ...
        try {
          if (atlas.generate()) {
            ...
            Donkeycraft.Logger.info(
              'Game',
              'Texture atlas generated successfully with ' +
                totalBlocks +
                ' textures'
            );
            ...
            Donkeycraft.Logger.warn(
              'Game',
              'Texture atlas generation failed — will use placeholder'
            );
          }
        } catch (e) {
          Donkeycraft.Logger.error(
            'Game',
            'Texture atlas creation failed: ' + e.message
          );
        }
      }
    };
  ...
  Donkeycraft.Game.prototype._compileShaderPrograms = function () {
    ...
    try {
      ...
      Donkeycraft.Logger.warn(
        'Game',
        'createProgramsFromDOM threw: ' + e.message
      );
    }
    ...
    if (!programs) {
      Donkeycraft.Logger.warn(
        'Game',
        'Shader sources not found — using fallback compilation'
      );
  ...
  Donkeycraft.Game.prototype._tick = function (dt, tickCount) {
    ...
    if (this._input && this._player && this._input.isKeyJustPressed('KeyC')) {
      try {
        ...
        if (this._eventBus) {
          try {
            this._eventBus.emit('gameMode:changed', { newMode: newMode });
          } catch (e2) {}
        }
    ...
    if (this._flyingSystem) {
      try {
        this._flyingSystem.tick(dt);
      } catch (e) {
        Donkeycraft.Logger.error('Game', 'Flying tick error: ' + e.message);
      }
    }
    if (this._movementSystem) {
      try {
        this._movementSystem.tick(dt);
      } catch (e) {
        Donkeycraft.Logger.error('Game', 'Movement tick error: ' + e.message);
      }
    }
    if (this._jumpSystem) {
      try {
        this._jumpSystem.tick(dt);
      } catch (e) {
        Donkeycraft.Logger.error('Game', 'Jump tick error: ' + e.message);
      }
    }
    // Vitals tick — consolidated into Player.tickVitals() for single source of truth
    if (this._player) {
      try {
        this._player.tickVitals(dt);
      } catch (e) {
        Donkeycraft.Logger.error(
          'Game',
          'Player vitals tick error: ' + e.message
        );
      }
    }
    ...
    if (this._hungerSystem) {
      try {
        this._hungerSystem.tick(dt);
      } catch (e) {
        Donkeycraft.Logger.error('Game', 'Hunger tick error: ' + e.message);
      }
    }
    ...
    if (this._worldStore && this._chunkManager) {
      ...
      if (this._autoSaveTimer >= Config.AUTO_SAVE_INTERVAL) {
        this._autoSaveTimer = 0;
        var worldName =
          this._levelData && this._levelData.getWorldName
        ...
        if (savePromise && typeof savePromise.then === 'function') {
          savePromise
            ...
            .catch(function (e) {
              Donkeycraft.Logger.warn(
                'Game',
                'Auto-save failed: ' + (e && e.message ? e.message : String(e))
              );
            });
        }
      }
    ...
    if (this._levelData && this._levelData.tickAutoSave) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'LevelData auto-save tick failed: ' + e.message
        );
      }
    }
      ...
      try {
        ...
        var groundCheckFn = function () {
          ...
          for (var gy = Math.floor(entPlayerPos.y + 2); gy >= 0; gy--) {
            var bid = selfTick._getBlockAt(
              Math.floor(entPlayerPos.x),
              gy,
              Math.floor(entPlayerPos.z)
            );
            if (
              bid &&
              Donkeycraft.BlockRegistry &&
              Donkeycraft.BlockRegistry.isSolid(bid)
            ) {
        ...
        Donkeycraft.Logger.error(
          'Game',
          'Entity engine tick failed: ' + e.message
        );
      }
    }
    ...
    if (this._entityManager && this._player) {
      try {
        ...
        Donkeycraft.Logger.error(
          'Game',
          'Entity manager tick failed: ' + e.message
        );
      }
    }
    ...
    if (
      Donkeycraft.Portal &&
      Donkeycraft.Portal._checkPlayerInPortal &&
      this._player
    ) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'Portal tick check failed: ' + e.message
        );
      }
    }
  ...
  Donkeycraft.Game.prototype._updatePlayer = function (dt) {
    ...
    if (kb && (Math.abs(kb.x) > 0.001 || Math.abs(kb.z) > 0.001)) {
      ...
      kb.z *= 0.9;
      kb.y *= 0.9;
      if (Math.abs(kb.x) < 0.001) {
        kb.x = 0;
      }
      if (Math.abs(kb.z) < 0.001) {
        kb.z = 0;
      }
      if (Math.abs(kb.y) < 0.001) {
        kb.y = 0;
      }
    }
  ...
  Donkeycraft.Game.prototype._updateChunks = function () {
    ...
    var playerMovedChunks =
      chunkX !== this._lastPlayerChunkX || chunkZ !== this._lastPlayerChunkZ;
    ...
    if (!playerMovedChunks) {
      ...
      for (var i = 0; i < dirtyChunks.length; i++) {
        this._terrainRenderer.markChunkDirty(
          dirtyChunks[i].chunkX,
          dirtyChunks[i].chunkZ
        );
      }
      return;
    }
    ...
    for (var j = 0; j < dirty.length; j++) {
      this._terrainRenderer.markChunkDirty(
        dirty[j].chunk.chunkX,
        dirty[j].chunk.chunkZ
      );
    }
  };
  ...
  Donkeycraft.Game.prototype._getSelectedBlockId = function () {
    ...
    if (
      this._player &&
      this._player.inventory &&
      this._player.hotbarSlot !== undefined
    ) {
  ...
  Donkeycraft.Game.prototype._processInteractions = function () {
    ...
    var direction = this._raycastSystem.getDirectionFromRotation(
      rot.yaw,
      rot.pitch
    );
    ...
    if (
      this._input.isMouseButtonPressed('left') &&
      result &&
      result.blockX !== undefined
    ) {
      if (this._blockActionSystem) {
        this._blockActionSystem.startBreaking(
          ...
          result.blockZ,
          {
            x: result.faceNormalX,
            y: result.faceNormalY,
            z: result.faceNormalZ,
          },
          this._player
        );
      }
    ...
    if (
      this._input.isMouseButtonPressed('right') &&
      result &&
      result.blockX !== undefined
    ) {
  ...
  Donkeycraft.Game.prototype._render = function (dt) {
    ...
    if (glErr !== this._gl.NO_ERROR) {
      Donkeycraft.Logger.error(
        'Game',
        'WebGL error after clear: 0x' + glErr.toString(16)
      );
    }
    ...
    if (
      ...
      try {
        ...
        var waterPlayerChanged =
          waterChunkX !== this._lastWaterChunkX ||
          waterChunkZ !== this._lastWaterChunkZ;
        var waterHasDirtyChunks =
          !this._dirtyChunksClearedThisFrame &&
          this._chunkManager &&
          this._chunkManager.getDirtyChunks().length > 0;

        if (waterPlayerChanged || waterHasDirtyChunks) {
          this._lastWaterChunkX = waterChunkX;
          this._lastWaterChunkZ = waterChunkZ;
          this._waterRenderer.updateMesh(
            waterChunkX,
            waterChunkZ,
            this._getBlockAt.bind(this)
          );
        }

        // Render water surface with reflection pass
        this._waterRenderer.render(
          this._camera,
          this._lighting,
          this._getBlockAt.bind(this)
        );
    ...
    if (this._renderWeather && this._weather && this._weatherRenderer) {
      try {
        ...
        if (
          particleDensity > 0 &&
          this._weatherRenderer.getParticleCount() === 0
        ) {
          this._weatherRenderer.spawnInitialParticles(
            Math.floor(500 * particleDensity)
          );
        }
        this._weatherRenderer.activate();
        this._weatherRenderer.update(
          dt,
          this._player ? this._player.getPosition() : { x: 0, y: 64, z: 0 }
        );
        this._weatherRenderer.render(
          this._camera,
          particleDensity,
          particleType
        );
    ...
    if (this._speedIndicator && this._movementSystem) {
      var activeMode = this._movementSystem.getActiveSpeedMode
        ? this._movementSystem.getActiveSpeedMode()
        : null;
      if (activeMode) {
    ...
    if (this._renderGUI && this._guiRenderer && this._canvas) {
      ...
      try {
        if (this._player) {
          health =
            this._player.getHealth !== undefined
              ? this._player.getHealth()
              : 20;
          maxHealth =
            this._player.getMaxHealth !== undefined
      ...
      try {
        if (this._hungerSystem) {
          food =
            this._hungerSystem.getFoodLevel !== undefined
              ? this._hungerSystem.getFoodLevel()
              : 20;
          maxFood = 20;
        }
      } catch (e) {
    ...
    if (this._todUI && this._player) {
      try {
        ...
        var todMode = this._player.getGameMode
          ? this._player.getGameMode()
          : null;
        ...
        Donkeycraft.Logger.warn(
          'Game',
          'TimeOfDayUI update error: ' + e.message
        );
      }
    }
    ...
    if (this._mapRenderer && this._player) {
      try {
        ...
        this._mapRenderer.renderFullMap(
          playerPos,
          playerRot.yaw,
          playerRot.pitch
        );
