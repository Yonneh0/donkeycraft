...
(function () {
     ...
     */
    var SURVIVAL_SPEEDS = [
        { id: 'sneak', emoji: '\uD83D\uDC0C', label: 'SNEAK', title: 'Sneak — 2.0 b/s (slow walk)' },
        { id: 'walk', emoji: '\uD83D\uDEB6', label: 'WALK', title: 'Walk — 5.0 b/s (normal speed)' },
        { id: 'run', emoji: '\uD83C\uDFC3', label: 'RUN', title: 'Run — 7.8 b/s (sprint)' }
    ];
    ...
    var CREATIVE_SPEEDS = [
        ...
        { id: 'walk', emoji: '\uD83D\uDEB6', label: 'WALK', title: 'Walk — 5.0 b/s (normal speed)' },
        { id: 'run', emoji: '\uD83C\uDFC3', label: 'RUN', title: 'Run — 7.8 b/s (sprint)' },
        { id: 'turbo', emoji: '\uD83D\uDE80', label: 'TURBO', title: 'Turbo — 20.0 b/s (4x walk speed)' }
    ];

    var FLYING_SPEEDS = [
        { id: 'normal', emoji: '\uD83D\uDDA5\uFE0F', label: 'NORMAL', title: 'Normal fly — 5.0 b/s' },
        { id: 'fast', emoji: '\u26A1', label: 'FAST', title: 'Fast fly — 10.0 b/s (2x normal)' },
        { id: 'turbo', emoji: '\uD83D\uDE80', label: 'TURBO', title: 'Turbo fly — 30.0 b/s (6x normal)' },
        { id: 'ultra', emoji: '\uD83D\uDCAF', label: 'ULTRA', title: 'Ultra fly — 60.0 b/s (12x normal)' }
    ];
    ...
    Donkeycraft.SpeedIndicator.prototype._buildDOM = function () {
        ...
        if (!container) {
            Donkeycraft.Logger.error('SpeedIndicator', 'No container element provided');
            return;
        }
        ...
        track.addEventListener('mousedown', function (e) { self._onMouseDown(e); }.bind(this));
        indicator.addEventListener('mousedown', function (e) { e.stopPropagation(); }.bind(this));

        // Touch support
        track.addEventListener('touchstart', function (e) { self._onTouchStart(e); }.bind(this), { passive: false });

        // Global mouse/touch move and up handlers
        document.addEventListener('mousemove', function (e) { self._onMouseMove(e); }.bind(this));
        document.addEventListener('mouseup', function (e) { self._onMouseUp(e); }.bind(this));
        document.addEventListener('touchmove', function (e) { self._onTouchMove(e); }.bind(this), { passive: false });
        document.addEventListener('touchend', function (e) { self._onTouchEnd(e); }.bind(this));
    };
    ...
    Donkeycraft.SpeedIndicator.prototype._positionStops = function () {
        ...
        for (var i = 0; i < count; i++) {
            ...
            if (count === 1) {
                ...
                percent = paddingPercent + (i / (count - 1)) * (100 - paddingPercent * 2);
            }
     ...
     */
    Donkeycraft.SpeedIndicator.prototype._setSliderIndex = function (index, animate) {
        var stops = this._elements.stops;
        if (!stops || stops.length === 0) return;
    ...
    Donkeycraft.SpeedIndicator.prototype._onTouchMove = function (e) {
        if (!this._dragState.isDragging || !e.touches || e.touches.length === 0) return;
        e.preventDefault();
    ...
    Donkeycraft.SpeedIndicator.prototype.setPlayer = function (player) {
        ...
        try {
            this._unsubscribeGameModeEvent = Donkeycraft.EventBus.onSafe('gameMode:changed', function (data) {
                if (data && data.newMode) {
                    self._onGameModeChanged(data.newMode);
                }
            });
        } catch (e) {
            Donkeycraft.Logger.warn('SpeedIndicator', 'Failed to subscribe to gameMode:changed: ' + e.message);
        }

        // Subscribe to flying state changes via global EventBus
        try {
            this._unsubscribeFlyingEvent = Donkeycraft.EventBus.onSafe('flying:stateChanged', function (data) {
            ...
            Donkeycraft.Logger.warn('SpeedIndicator', 'Failed to subscribe to flying:stateChanged: ' + e.message);
        }
    ...
    Donkeycraft.SpeedIndicator.prototype._cleanupEvents = function () {
        if (this._unsubscribeGameModeEvent) {
            try { this._unsubscribeGameModeEvent(); } catch (e) { }
            this._unsubscribeGameModeEvent = null;
        }
        if (this._unsubscribeFlyingEvent) {
            try { this._unsubscribeFlyingEvent(); } catch (e) { }
            this._unsubscribeFlyingEvent = null;
        }
    };
    ...
    Donkeycraft.SpeedIndicator.prototype.update = function (activeMode) {
        ...
        if (this._player && this._player.flyEnabled && this._gameMode === 'creative') {
            expectedFlying = true;
        }
    ...
    Donkeycraft.SpeedIndicator.prototype.setSliderValue = function (index) {
        ...
        var safeIndex = Math.max(0, Math.min(index, this._elements.stops.length - 1));
