...
(function () {
  ...
  Donkeycraft.HealthBar = function (container, player) {
    ...
    // Bind + build
    var self = this;
    this._onHealthChanged = function (data) {
      self.updateFromHealth(data);
    };
    ...
    if (globalBus) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'HealthBar',
          'Failed to subscribe to health:changed: ' + e.message
        );
      }
    } else {
      Donkeycraft.Logger.warn(
        'HealthBar',
        'No global EventBus instance available — health bar will not receive updates'
      );
    }
  };
   ...
   */
  Donkeycraft.HealthBar.prototype._getHeartFillLevel = function (
    health,
    index
  ) {
  ...
  Donkeycraft.HealthBar.prototype._getHeartSVG = function (fillLevel) {
    ...
    if (fillLevel === 0) {
      ...
      return (
        '<svg viewBox="0 0 16 18" class="dk-heart dk-heart-empty-svg">' +
    ...
    return (
      '<svg viewBox="0 0 16 18" class="dk-heart dk-heart-fill">' +
      '<defs>' +
      '<clipPath id="' +
      clipId +
      '">' +
   ...
   */
  Donkeycraft.HealthBar.prototype._animateOnHealthChange = function (
    delta,
    oldHealth,
    newHealth
  ) {
   ...
   */
  Donkeycraft.HealthBar.prototype._flashDamagedHearts = function (
    delta,
    oldHealth,
    newHealth
  ) {
   ...
   */
  Donkeycraft.HealthBar.prototype._flashHealedHearts = function (
    delta,
    oldHealth,
    newHealth
  ) {
   ...
   */
  Donkeycraft.HealthBar.prototype._spawnHealthTextAt = function (
    delta,
    heartIndex
  ) {
   ...
   */
  Donkeycraft.HealthBar.prototype._updateRedOverlay = function (
    health,
    maxHealth
  ) {
  ...
  Donkeycraft.HealthBar.prototype.resetUI = function () {
    ...
    for (var j = 0; j < this._heartContainers.length; j++) {
      if (this._heartContainers[j]) {
        this._heartContainers[j].classList.remove(
          'dk-heart-flash',
          'dk-heart-heal-flash'
        );
      }
