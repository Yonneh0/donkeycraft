...
(function () {
  ...
  Donkeycraft.TimeOfDayUI = function (options) {
    ...
    this._onTimeChange =
      typeof options.onTimeChange === 'function' ? options.onTimeChange : null;
  ...
  Donkeycraft.TimeOfDayUI.prototype._updateTimeDial = function () {
    try {
      ...
      this._timeDialEl.style.background =
        'radial-gradient(circle at 40% 40%, rgba(30, 35, 55, 0.92), rgba(15, 18, 30, 0.95))';
      ...
      if (this._timeDialDaylight) {
        this._timeDialDaylight.style.background =
          'conic-gradient(from ' +
          sunriseAngle +
          'deg, rgba(200, 220, 255, 0.45), rgba(200, 220, 255, 0.45) ' +
          dayArcEnd +
          'deg, transparent ' +
          dayArcEnd +
          'deg, transparent ' +
      ...
      this._dialPointer.style.transform =
        'translate(-50%, -50%) rotate(' +
        pointerAngle +
        'deg) translateX(16px)';

      // Emoji marker: sun during day, moon during night
      this._dialPointer.textContent = isDaytime
        ? '\u2600\uFE0F'
        : '\uD83C\uDF19';
      ...
      var h12 = hour % 12 || 12;
      this._dialText.textContent =
        h12 + ':' + (minute < 10 ? '0' : '') + minute;
      ...
      Donkeycraft.Logger.warn(
        'TimeOfDayUI',
        'Time dial update error: ' + e.message
      );
    }
  };
  ...
  Donkeycraft.TimeOfDayUI.prototype._destroySlider = function () {
    ...
    if (this._handlers._onSliderInput && this._sliderInput) {
      this._sliderInput.removeEventListener(
        'input',
        this._handlers._onSliderInput
      );
    }
    if (this._handlers._onFreezeClick && this._freezeBtn) {
      this._freezeBtn.removeEventListener(
        'click',
        this._handlers._onFreezeClick
      );
    }
  ...
  Donkeycraft.TimeOfDayUI.prototype.destroy = function () {
    ...
    if (this._timeDialEl && this._handlers._onDialClick) {
      this._timeDialEl.removeEventListener(
        'click',
        this._handlers._onDialClick
      );
    }
