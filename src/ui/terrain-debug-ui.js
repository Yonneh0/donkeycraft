...
(function () {
  ...
  Donkeycraft.TerrainDebugUI.prototype._populateBiomeSelector = function () {
    ...
    var biomes = Donkeycraft.BiomeRegistry
      ? Donkeycraft.BiomeRegistry.getAllBiomes()
      : [];
    ...
    for (var i = 0; i < biomes.length; i++) {
      ...
      opt.textContent =
        biomes[i].name != null ? biomes[i].name : 'Biome #' + (i + 1);
      sel.appendChild(opt);
    }
  ...
  Donkeycraft.TerrainDebugUI.prototype._renderChunkGrid = function () {
    ...
    if (rows * cols > MAX_GRID_CELLS) {
      console.warn(
        '[TerrainDebugUI] Grid too large, clamping to',
        MAX_GRID_CELLS,
        'cells'
      );
      return;
    ...
    var chunkPos = this._renderer.getChunkPosition
      ? this._renderer.getChunkPosition()
      : { x: 0, z: 0 };
  ...
  Donkeycraft.TerrainDebugUI.prototype._getTopBlockColor = function (chunk) {
    ...
    var colors = Donkeycraft.BlockColors
      ? Donkeycraft.BlockColors.getAllColors()
      : null;
  ...
  Donkeycraft.TerrainDebugUI.prototype._wireUpHandlers = function () {
    ...
    for (var i = 0; i < navIds.length; i++) {
      (function (id, d) {
        ...
        btn.addEventListener('click', function () {
          ...
          var pos = self._renderer.getChunkPosition
            ? self._renderer.getChunkPosition()
            : { x: 0, z: 0 };
    ...
    var chunkBtnActions = {
      'btn-expand-n': function () {
        if (self._renderer) {
          var r = self._renderer.getRadii();
          r.n++;
          self._renderer.setRadii(r);
          if (self._renderer.regenerateTerrain)
            self._renderer.regenerateTerrain();
        }
      },
      'btn-contract-n': function () {
        if (self._renderer) {
          var r = self._renderer.getRadii();
          if (r.n > 0) {
            r.n--;
  ...
  Donkeycraft.TerrainDebugUI.prototype._updateStatsDisplay = function () {
    ...
    if (chunkPosEl) {
      var pos = this._renderer.getChunkPosition
        ? this._renderer.getChunkPosition()
        : { x: 0, z: 0 };
    ...
    if (chunkCountEl) {
      var count = this._renderer.getChunkCount
        ? this._renderer.getChunkCount()
        : 0;
    ...
    if (genTimeEl) {
      var gt = this._renderer.getGenerationTime
        ? this._renderer.getGenerationTime()
        : 0;
    ...
    if (statsEl) {
      var chunkCount = this._renderer.getChunkCount
        ? this._renderer.getChunkCount()
        : 0;
      var genTime = this._renderer.getGenerationTime
        ? this._renderer.getGenerationTime()
        : 0;
    ...
    if (fpsEl) {
      var fps = this._renderer.getCurrentFps
        ? this._renderer.getCurrentFps()
        : 0;
