...
(function () {
   ...
   */
  Donkeycraft.LoadingScreen.prototype._addHistoryEntry = function (
    label,
    duration
  ) {
  ...
  Donkeycraft.LoadingScreen.prototype._endPhase = function (phase) {
    ...
    var label = phase.replace(/-/g, ' ').replace(/\b\w/g, function (c) {
  ...
  Donkeycraft.LoadingScreen.prototype.updateProgress = function (percent) {
    ...
    if (p % 25 === 0 && p > 0 && p > oldProgress) {
      this._tipIndex =
        (this._tipIndex + 1) % Donkeycraft.LoadingScreenTips.length;
      this._tipEl.textContent =
        '[Tip] ' + Donkeycraft.LoadingScreenTips[this._tipIndex];
    }
  };
