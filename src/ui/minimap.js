...
(function () {
  ...
  Donkeycraft.MinimapUI = function (options) {
    ...
    /** @type {number} @private */
    this._minimapRadius =
      Config && Config.MAP_MINIMAP_RADIUS ? Config.MAP_MINIMAP_RADIUS : 32;
    ...
    try {
      ...
      if (!this._canvas) {
        Donkeycraft.Logger.warn(
          'MinimapUI',
          'Minimap canvas element #dk-minimap-canvas not found in DOM'
        );
        return false;
      }

      var minimapSize =
        Config && Config.MAP_MINIMAP_SIZE ? Config.MAP_MINIMAP_SIZE : 150;
  ...
  Donkeycraft.MinimapUI._getSurfaceBlock = function (chunkManager, wx, wz) {
    ...
    if (
      surfaceMap &&
      surfaceMap[localX] &&
      surfaceMap[localX][localZ] !== undefined
    ) {
    ...
    try {
      var worldHeight =
        Config && Config.WORLD_HEIGHT ? Config.WORLD_HEIGHT : 256;
      ...
      Donkeycraft.Logger.warn(
        'MinimapUI',
        'Failed to read block at [' + localX + ',' + localZ + ']: ' + e.message
      );
    }

    // Cache result in surface map (consistent format with chunk-manager: {y, blockId})
    chunk._mapSurfaceMap[localX][localZ] =
  ...
  Donkeycraft.MinimapUI.prototype.render = function (playerPos, yaw) {
    ...
    for (var bx = startBlockX; bx <= endBlockX; bx++) {
      for (var bz = startBlockZ; bz <= endBlockZ; bz++) {
        ...
        var blockId = Donkeycraft.MinimapUI._getSurfaceBlock(
          this._chunkManager,
          worldX,
          worldZ
        );
   ...
   * @private
   */
  Donkeycraft.MinimapUI.prototype._renderCompassRing = function (
    ctx,
    cx,
    cy,
    radius
