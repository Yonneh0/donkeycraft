...
(function () {
    ...
    var events = [
      {
        ...
        handler: function (data) {
          if (data && data.entity) {
            self._onEntitySpawned(data.entity);
          }
        },
      },
      {
        ...
        handler: function (data) {
          if (data && data.entity) {
            self._onEntityDespawned(data.entity);
          }
        },
      },
      {
        ...
        handler: function (data) {
          if (data && data.target) {
            self._updateNametag(data.target);
          }
        },
      },
    ];
  ...
  Donkeycraft.NametagRenderer.prototype._getEntities = function () {
    ...
    if (
      this._entityManager.getAllEntities &&
      typeof this._entityManager.getAllEntities === 'function'
    ) {
      return this._entityManager.getAllEntities();
    }
    // Fall back to getEntities() method (some engines use this name)
    if (
      this._entityManager.getEntities &&
      typeof this._entityManager.getEntities === 'function'
    ) {
      return this._entityManager.getEntities();
    }
    // Try direct property access to _entities
    if (
      this._entityManager._entities &&
      typeof this._entityManager._entities === 'object'
    ) {
      var result = [];
   ...
   * @private
   */
  Donkeycraft.NametagRenderer.prototype._worldToScreen = function (
    worldPos,
    camera,
    gl
  ) {
    ...
    var fov = camera.getFov
      ? camera.getFov()
      : (Donkeycraft.Config && Donkeycraft.Config.FOV) || 70;
   ...
   * @private
   */
  Donkeycraft.NametagRenderer.prototype._isOnScreen = function (
    entity,
    camera
  ) {
    var pos = entity.getPosition ? entity.getPosition() : null;
   ...
   * @private
   */
  Donkeycraft.NametagRenderer.prototype._createNametagElement = function (
    entity
  ) {
  ...
  Donkeycraft.NametagRenderer.prototype._updateNametag = function (entity) {
    ...
    if (screenPos.distance) {
      ...
      if (screenPos.distance > maxDist * 0.7) {
        opacity = Math.max(
          NAMETAG_MIN_OPACITY,
          1.0 - (screenPos.distance - maxDist * 0.7) / (maxDist * 0.3)
        );
      }
    }
    ...
    if (
      hpFillEl &&
      entity.getHealth !== undefined &&
      entity.getMaxHealth !== undefined
    ) {
      var currentHealth =
        typeof entity.getHealth === 'function' ? entity.getHealth() : 0;
      var maxHealth =
        typeof entity.getMaxHealth === 'function' ? entity.getMaxHealth() : 20;
   ...
   * @private
   */
  Donkeycraft.NametagRenderer.prototype._removeNametagElement = function (
    entityId
  ) {
