...
(function () {
  ...
  Donkeycraft.DebugOverlay.prototype._ensureDOM = function () {
    ...
    if (!this._element) {
      Donkeycraft.Logger.warn(
        'DebugOverlay',
        'DOM element #dk-debug-overlay not found'
      );
      return false;
    }
    ...
    for (let g = 0; g < statGroups.length; g++) {
      ...
      for (let b = 0; b < group.buttons.length; b++) {
        ...
        btn.addEventListener('click', function (e) {
          e.preventDefault();
          e.stopPropagation();
          self._onDebugControlClick(
            this.getAttribute('data-action'),
            this.getAttribute('data-args')
          );
        });
        row.appendChild(btn);
      }
  ...
  Donkeycraft.DebugOverlay.prototype._buildDebugTerrainToggle = function () {
    ...
    this._terrainToggleBtn.addEventListener(
      'click',
      (function (self) {
        return function (e) {
          e.preventDefault();
  ...
  Donkeycraft.DebugOverlay.prototype.getChunkInfo = function () {
    ...
    try {
      ...
      Donkeycraft.Logger.warn(
        'DebugOverlay',
        `Failed to read RENDER_DISTANCE: ${e.message}`
      );
    }
    ...
    try {
      if (
        this._chunkManager &&
        typeof this._chunkManager.getAllChunks === 'function'
      ) {
      ...
      Donkeycraft.Logger.warn(
        'DebugOverlay',
        `Failed to count chunks: ${e.message}`
      );
    }
  ...
  Donkeycraft.DebugOverlay.prototype.getPlayerCoords = function () {
    ...
    if (this._player) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to read player position: ${e.message}`
        );
      }
      ...
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to read player rotation: ${e.message}`
        );
      }

      try {
        mode = this._player.getGameMode
          ? this._player.getGameMode()
          : 'survival';
      } catch (e) {
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to read game mode: ${e.message}`
        );
      }
    }
  ...
  Donkeycraft.DebugOverlay.prototype.getBiomeName = function () {
    ...
    try {
      ...
      Donkeycraft.Logger.warn(
        'DebugOverlay',
        `Failed to read player position for biome: ${e.message}`
      );
      return 'Unknown';
    }

    const chunkSize =
      Donkeycraft.Config && Donkeycraft.Config.CHUNK_SIZE
        ? Donkeycraft.Config.CHUNK_SIZE
        : 16;
  ...
  Donkeycraft.DebugOverlay.prototype.getGameMode = function () {
    if (this._player && typeof this._player.getGameMode === 'function') {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to get game mode: ${e.message}`
        );
      }
    }
  ...
  Donkeycraft.DebugOverlay.prototype.getLightLevels = function () {
    ...
    if (this._player && this._chunkManager) {
      try {
        ...
        const chunk = this._chunkManager.getChunk
          ? this._chunkManager.getChunk(chunkX, chunkZ)
          : null;
        if (chunk && typeof chunk.getSkyLight === 'function') {
          skyLight = Math.max(
            0,
            Math.min(15, chunk.getSkyLight(localX, localY, localZ))
          );
          blockLight = Math.max(
            0,
            Math.min(
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to read light levels: ${e.message}`
        );
      }
    }
  ...
  Donkeycraft.DebugOverlay.prototype._getRenderStats = function () {
    ...
    try {
      const raw =
        this._terrainRenderer.getRenderStats &&
        this._terrainRenderer.getRenderStats();
      if (raw) {
        stats.chunksRendered =
          typeof raw.chunksRendered === 'number' ? raw.chunksRendered : 0;
        stats.meshesBuilt =
          typeof raw.meshesBuilt === 'number' ? raw.meshesBuilt : 0;
        stats.drawCalls = typeof raw.drawCalls === 'number' ? raw.drawCalls : 0;
      }
    } catch (e) {
      Donkeycraft.Logger.warn(
        'DebugOverlay',
        `Failed to get render stats: ${e.message}`
      );
    }
  ...
  Donkeycraft.DebugOverlay.prototype._getRendererToggles = function () {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to read renderer toggle "${key}": ${e.message}`
        );
        toggles[key] = true;
      }
    }
  ...
  Donkeycraft.DebugOverlay.prototype._getDeltaTime = function () {
    if (this._timer && typeof this._timer.getDeltaTime === 'function') {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to get delta time: ${e.message}`
        );
      }
    }
    return 0;
  ...
  Donkeycraft.DebugOverlay.prototype._getFPS = function () {
    if (this._timer && typeof this._timer.getFPS === 'function') {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to get FPS from timer: ${e.message}`
        );
      }
    }
    return this._fps;
  ...
  Donkeycraft.DebugOverlay.prototype._renderDOM = function (data) {
    ...
    this._cache.renderDist.textContent = this._escapeHtml(
      String(data.chunkInfo.renderDistance)
    );
  ...
  Donkeycraft.DebugOverlay.prototype.update = function (player, chunkManager) {
    ...
    if (this._eventBus) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to emit debug:update event: ${e.message}`
        );
      }
    }
  };
  ...
  Donkeycraft.DebugOverlay.prototype.startListening = function () {
    ...
    if (this._timer && typeof this._timer.onRender === 'function') {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to subscribe to timer.onRender: ${e.message}`
        );
      }
    }
    ...
    try {
      ...
      Donkeycraft.Logger.error(
        'DebugOverlay',
        `Failed to start render loop: ${e.message}`
      );
    }
  };
   ...
   */
  Donkeycraft.DebugOverlay.prototype._onDebugControlClick = function (
    action,
    args
  ) {
    var self2 = this;
    ...
    try {
      if (typeof Donkeycraft !== 'undefined' && Donkeycraft._xpInstance) {
        ...
        window._dkGame &&
        window._dkGame._player &&
        window._dkGame._player.experience
      ) {
    ...
    switch (action) {
      case 'health': {
        if (!player) {
          Donkeycraft.Logger.warn('DebugOverlay', 'Player not available');
          break;
        }
        var currentHealth =
          player.getHealth() !== undefined ? player.getHealth() : 20;
      ...
      case 'xpPoints': {
        if (!xp) {
          Donkeycraft.Logger.warn(
            'DebugOverlay',
            'Experience system not available'
          );
          break;
        }
      ...
      default:
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          'Unknown debug control action: ' + action
        );
    }
  };
  ...
  Donkeycraft.DebugOverlay.prototype._onDebugTerrainToggle = function () {
    ...
    try {
      if (!Donkeycraft.DebugTerrainGenerator) {
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          'DebugTerrainGenerator not available'
        );
        return;
      }
      ...
      Donkeycraft.Logger.error(
        'DebugOverlay',
        'Failed to toggle debug terrain: ' + e.message
      );
    }
  };
  ...
  Donkeycraft.DebugOverlay.prototype.stopListening = function () {
    if (this._unsubscribeRender) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'DebugOverlay',
          `Failed to stop render loop: ${e.message}`
        );
      }
      this._unsubscribeRender = null;
    }
