...
(function () {
  ...
  Donkeycraft.FurnaceUI.prototype._buildDOM = function () {
    ...
    this._fuelSlotEl.innerHTML =
      '<span class="dk-furnace-placeholder">Fuel</span>';
    ...
    this._inputSlotEl.innerHTML =
      '<span class="dk-furnace-placeholder">Input</span>';
    ...
    this._outputSlotEl.innerHTML =
      '<span class="dk-furnace-placeholder">Output</span>';
  ...
  Donkeycraft.FurnaceUI.prototype._updateSlotDisplay = function (index) {
    ...
    if (!stack || stack.isEmpty()) {
      ...
      el.innerHTML =
        '<span class="dk-furnace-placeholder">' +
        (labels[index] || '') +
        '</span>';
    } else {
  ...
  Donkeycraft.FurnaceUI.prototype.setSlot = function (index, stack) {
    ...
    if (stack !== null && !(stack instanceof Donkeycraft.ItemStack))
      return false;
  ...
  Donkeycraft.FurnaceUI.getSmeltableItems = function () {
    if (
      !Donkeycraft.RecipeRegistry ||
      typeof Donkeycraft.RecipeRegistry.getSmeltRecipes !== 'function'
    ) {
      return [];
    }
    try {
  ...
  Donkeycraft.FurnaceUI.prototype._getFuelBurnTime = function (stack) {
    if (!stack || typeof stack.isEmpty !== 'function' || stack.isEmpty())
      return 0;
  ...
  Donkeycraft.FurnaceUI.prototype._getSmeltRecipe = function (itemId) {
    if (
      !Donkeycraft.RecipeRegistry ||
      typeof Donkeycraft.RecipeRegistry.getSmeltRecipe !== 'function'
    )
      return null;
    try {
  ...
  Donkeycraft.FurnaceUI.prototype._produceOutput = function () {
    ...
    if (
      typeof outputBlockId !== 'number' ||
      !Number.isFinite(outputBlockId) ||
      outputBlockId <= 0
    ) {
      return null;
    }
