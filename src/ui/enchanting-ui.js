...
(function () {
  ...
  Donkeycraft.EnchantingUI.prototype._buildDOM = function () {
    ...
    this._inputSlotEl.innerHTML =
      '<span class="dk-enchant-placeholder">Item</span>';
    ...
    this._lapisSlotEl.innerHTML =
      '<span class="dk-enchant-placeholder">Lapis</span>';
    ...
    this._outputSlotEl.innerHTML =
      '<span class="dk-enchant-placeholder">Result</span>';
  ...
  Donkeycraft.EnchantingUI.prototype._updateSlotDisplay = function (slotName) {
    ...
    switch (slotName) {
      case 'input':
        this._inputSlotEl =
          this._inputSlotEl ||
          this._container.querySelector('.dk-enchant-input');
      ...
      case 'output':
        this._outputSlotEl =
          this._outputSlotEl ||
          this._container.querySelector('.dk-enchant-output');
      ...
      case 'lapis':
        this._lapisSlotEl =
          this._lapisSlotEl ||
          this._container.querySelector('.dk-enchant-lapis');
      ...
      if (costSpan) {
        if (opt) {
          ...
          costSpan.style.color =
            this._playerLevels >= opt.cost ? '#4a9' : '#f84';
        } else {
  ...
  Donkeycraft.EnchantingUI.prototype._updateCostDisplay = function () {
    ...
    this._levelCostEl.style.color =
      this._playerLevels >= cost ? '#4a9' : '#f84';
  };
  ...
  Donkeycraft.EnchantingUI.prototype._generateEnchantmentOptions = function () {
    ...
    var itemBlockId = this._inputSlot.getItemId
      ? this._inputSlot.getItemId()
      : 0;
    ...
    while (this._enchantOptions.length < 3 && attempts < maxAttempts) {
      ...
      var baseCost = registry.calculateLevelCost
        ? registry.calculateLevelCost(enchant.id, randomLevel)
        : randomLevel;
      ...
      if (
        this._enchantOptions.length > 0 &&
        cost <= this._enchantOptions[this._enchantOptions.length - 1].cost
      ) {
  ...
  Donkeycraft.EnchantingUI.prototype.takeResult = function () {
    ...
    var lapisCost =
      (this._enchantOptions[this._selectedOption] || {}).lapisCost || 1;
  ...
  Donkeycraft.EnchantingUI.prototype.setPlayerLevels = function (levels) {
    if (typeof levels !== 'number' || !Number.isInteger(levels) || levels < 0)
      return false;
