...
(function () {
  ...
  Donkeycraft.HungerBar = function (container, hunger) {
    ...
    // --- Bind + build ---
    var self = this;
    this._onHungerChanged = function (data) {
      self.updateFromFood(data);
    };
    ...
    if (globalBus) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'HungerBar',
          'Failed to subscribe to hunger:changed: ' + e.message
        );
      }
    } else {
      Donkeycraft.Logger.warn(
        'HungerBar',
        'No global EventBus instance available — hunger bar will not receive updates'
      );
    }
  };
   ...
   */
  Donkeycraft.HungerBar.prototype._getDrumstickFillLevel = function (
    food,
    index
  ) {
   ...
   * @returns {number} Fill level from 0 (empty) to 10 (full).
   */
  Donkeycraft.HungerBar.prototype._getDropFillLevel = function (
    hydration,
    index
  ) {
  ...
  Donkeycraft.HungerBar.prototype._getDrumstickSVG = function (fillLevel) {
    ...
    return (
      '<svg viewBox="0 0 1024 1024" class="dk-drumstick dk-drumstick-fill">' +
      '<defs>' +
      '<clipPath id="' +
      clipId +
      '">' +
  ...
  Donkeycraft.HungerBar.prototype._getDropSVG = function (fillLevel) {
    ...
    var highlightPath =
      'M5.5 12.5a4.5 4.5 0 0 0 4 4.4c-0.8-1.2-1.3-2.6-1.3-4.1 0-1.5 0.5-2.9 1.3-4.1a4.5 4.5 0 0 0-4 3.8z';

    if (fillLevel === 0) {
      // Empty drop — outline only
      return (
        '<svg viewBox="0 0 18 20" class="dk-drop dk-drop-empty-svg">' +
        '<path d="' +
        dropPath +
        '" fill="none" stroke="#4a90d9" stroke-width="1.2" stroke-linejoin="round"/>' +
        '</svg>'
      );
    }
    ...
    return (
      '<svg viewBox="0 0 18 20" class="dk-drop dk-drop-fill">' +
      '<defs>' +
      '<clipPath id="' +
      clipId +
      '">' +
  ...
  Donkeycraft.HungerBar.prototype.updateFromFood = function (data) {
    ...
    var newFood = Math.max(
      0,
      Math.min(MAX_FOOD, data.foodLevel != null ? data.foodLevel : oldFood)
    );

    // Clamp hydration to valid range [0, MAX_HYDRATION]
    var newHydration = Math.max(
      0,
      Math.min(
        MAX_HYDRATION,
        data.hydration != null ? data.hydration : this._prevHydration
      )
    );
    ...
    if (hydrationDelta !== 0) {
      this._animateOnHydrationChange(
        hydrationDelta,
        this._prevHydration,
        newHydration
      );
   ...
   */
  Donkeycraft.HungerBar.prototype._calculateShakeIntensity = function (
    currentLevel,
    maxLevel
  ) {
   ...
   * @param {number} maxLevel - Maximum level value.
   */
  Donkeycraft.HungerBar.prototype._triggerShake = function (
    currentLevel,
    maxLevel
  ) {
    ...
    var keyframes = [
      ...
      {
        transform: 'translateX(' + intensity.amplitude * 0.5 + 'px)',
        offset: 0.02,
      },
    ];
   ...
   */
  Donkeycraft.HungerBar.prototype._animateOnFoodChange = function (
    delta,
    oldFood,
    newFood
  ) {
   ...
   */
  Donkeycraft.HungerBar.prototype._animateOnHydrationChange = function (
    delta,
    oldHydration,
    newHydration
  ) {
   ...
   */
  Donkeycraft.HungerBar.prototype._pulseEatenDrumsticks = function (
    foodGain,
    oldFood,
    newFood
  ) {
    ...
    if (changedIndices.length > 0) {
      var centerIdx = Math.round(
        (changedIndices[0] + changedIndices[changedIndices.length - 1]) / 2
      );
   ...
   */
  Donkeycraft.HungerBar.prototype._pulseDrankDrops = function (
    hydrationGain,
    oldHydration,
    newHydration
  ) {
   ...
   */
  Donkeycraft.HungerBar.prototype._dimDepletedDrumsticks = function (
    delta,
    oldFood,
    newFood
  ) {
   ...
   */
  Donkeycraft.HungerBar.prototype._dimDepletedDrops = function (
    delta,
    oldHydration,
    newHydration
  ) {
   ...
   */
  Donkeycraft.HungerBar.prototype._spawnHungerTextAt = function (
    delta,
    drumIndex
  ) {
   ...
   */
  Donkeycraft.HungerBar.prototype._spawnHydrationTextAt = function (
    delta,
    dropIndex
  ) {
   ...
   * @param {number} hydrationLevel - Current hydration level (0-20).
   */
  Donkeycraft.HungerBar.prototype._updateBlueOverlay = function (
    hydrationLevel
  ) {
  ...
  Donkeycraft.HungerBar.prototype.resetUI = function () {
    ...
    for (var j = 0; j < this._drumstickContainers.length; j++) {
      if (this._drumstickContainers[j]) {
        this._drumstickContainers[j].classList.remove(
          'dk-drumstick-eat-pulse',
          'dk-drumstick-dim'
        );
      }
    }
    ...
    for (var k = 0; k < this._dropContainers.length; k++) {
      if (this._dropContainers[k]) {
        this._dropContainers[k].classList.remove(
          'dk-drop-drink-pulse',
          'dk-drop-dim'
        );
      }
