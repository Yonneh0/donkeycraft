...
(function () {
  ...
  Donkeycraft.CraftingGrid = function (container, recipeRegistry) {
    ...
    this._recipeRegistry =
      recipeRegistry !== undefined
        ? recipeRegistry
        : Donkeycraft.RecipeRegistry || null;
  ...
  Donkeycraft.CraftingGrid.prototype._buildDOM = function () {
    ...
    this._resultElement.addEventListener(
      'mousedown',
      (function () {
        return function () {
          self._onResultClick();
  ...
  Donkeycraft.CraftingGrid.prototype._onSlotClick = function (index) {
    ...
    if (Donkeycraft.EventBus) {
      try {
        Donkeycraft.EventBus.emit('crafting:slotClick', {
          index: index,
          stack: stack,
        });
      } catch (e) {
    ...
    try {
      ...
      if (Donkeycraft.Logger)
        Donkeycraft.Logger.error(
          'CraftingGrid: Recipe matching failed: ' + e.message
        );
      return null;
    }
  };
  ...
  Donkeycraft.CraftingGrid.prototype._tryMatchRecipe = function (gridChanged) {
    ...
    if (recipe) {
      try {
        ...
        if (Donkeycraft.Logger)
          Donkeycraft.Logger.error(
            'CraftingGrid: Failed to create result ItemStack: ' + e.message
          );
        this._resultStack = null;
      }
    } else {
  ...
  Donkeycraft.CraftingGrid.prototype._updateResultDisplay = function () {
    ...
    if (this._resultStack && !this._resultStack.isEmpty()) {
      itemEl.textContent = this._getItemDisplayChar(
        this._resultStack.getItemId()
      );
      this._resultElement.className =
        'dk-crafting-result dk-crafting-result-valid';
    } else {
      itemEl.textContent = '';
      this._resultElement.className = 'dk-crafting-result';
    }
   ...
   */
  Donkeycraft.CraftingGrid.prototype.handleKeyPress = function (
    key,
    hotbarItem
  ) {
