...
(function() {
    ...
    Donkeycraft.XPBar.prototype._subscribeToEvents = function() {
        ...
        if (globalBus) {
            try {
                ...
                Donkeycraft.Logger.warn('XPBar', 'Failed to subscribe to xp:changed: ' + e.message);
            }
        } else {
            Donkeycraft.Logger.warn('XPBar', 'No global EventBus instance available — XP bar will not receive updates');
        }
    };
    ...
    Donkeycraft.XPBar.prototype._buildDOM = function() {
        ...
        particleContainer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:4;overflow:visible;';
    ...
    Donkeycraft.XPBar.prototype.updateFromExperience = function(data) {
        ...
        var oldTier = (this._currentTier >= 0) ? Donkeycraft.XPBar.LEVEL_TIERS[this._currentTier] : null;
    ...
    Donkeycraft.XPBar.prototype._animateFill = function(progress, tier) {
        ...
        if (tier && tier.glow && tier.glow !== 'transparent' && tier.glow !== 'multi-color') {
            this._fill.style.boxShadow = 'inset 0 1px 0 rgba(255,255,255,0.2), 0 0 8px ' + tier.glow;
        } else if (tier && tier.glow === 'multi-color') {
            this._fill.style.boxShadow = 'inset 0 1px 0 rgba(255,255,255,0.2), 0 0 8px rgba(255,102,204,0.6)';
        } else {
            this._fill.style.boxShadow = 'inset 0 1px 0 rgba(255,255,255,0.2), 0 0 4px rgba(200,200,200,0.3)';
        }
    };
    ...
    Donkeycraft.XPBar.prototype._triggerLevelBurst = function() {
        ...
        burst.style.cssText = 'border-color:' + burstColor + ';animation:level-burst 400ms ease-out forwards;';
    ...
    Donkeycraft.XPBar.prototype._spawnParticles = function(count, tier) {
        ...
        var containerRect = this._container ? this._container.getBoundingClientRect() : null;
        ...
        for (var i = 0; i < count; i++) {
            ...
            particle.style.cssText = 'left:50%;top:50%;background:' + particleColor + ';opacity:1;animation:particle-fall ' + (400 + Math.random() * 200) + 'ms ease-out ' + (i * 15) + 'ms forwards;--dx:' + dx + 'px;--dy:' + dy + 'px;';
            ...
            }).bind(this, particle), totalDuration);
        }
    };
    ...
    Donkeycraft.XPBar.prototype._updateBadgeStatic = function(level, tier) {
        ...
        // Glow ring
        if (tier.glow && tier.glow !== 'transparent' && tier.glow !== 'multi-color') {
    ...
    Donkeycraft.XPBar.prototype._updateAmbientAnimation = function(newTier, oldTier) {
        ...
        if (ambient === 'pulse-glow') {
            this._badgeBg.style.animation = 'badge-pulse-glow 2s ease-in-out infinite';
            this._badgeBg.style.setProperty('--glow-color', glowColor);
        } else if (ambient === 'intensified-pulse') {
            this._badgeBg.style.animation = 'intensified-pulse 1.5s ease-in-out infinite';
            ...
            this._badgeBg.style.animation = 'golden-spectacle 2s ease-in-out infinite';
        }
    ...
    Donkeycraft.XPBar.prototype._ensureAmbientRing = function(tier) {
        ...
        var existing = this._badgeContainer.querySelector('.dk-xp-badge-ambient-ring');
    ...
    Donkeycraft.XPBar.prototype._ensureSparkElements = function(tier) {
        ...
        var badgeSize = this._badgeBg ? parseInt(this._badgeBg.style.width) || 48 : 48;
        ...
        for (var s = 0; s < sparkCount; s++) {
            ...
            spark.style.cssText += ';position:absolute;left:50%;top:50%;' +
                'animation:spark-float ' + (0.8 + Math.random() * 0.4) + 's linear infinite ' +
                (s * (1000 / sparkCount)) + 'ms;' +
    ...
    Donkeycraft.XPBar.prototype.resetUI = function() {
        ...
        var bursts = this._container ? this._container.querySelectorAll('.dk-xp-level-burst') : [];
        ...
        if (this._badgeContainer) {
            var ambientRing = this._badgeContainer.querySelector('.dk-xp-badge-ambient-ring');
