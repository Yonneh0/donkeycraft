...
(function () {
    ...
    /**
     * _getKeyLabel — converts a key code (e.g., 'KeyW', 'Digit1') to a human-readable label.
     * @private
     * @param {string} keyCode - The key code from Config.KEYBINDS.
     * @returns {string} Display label with styled key text.
     */
    ...
    Donkeycraft.KeybindingsPanel.prototype._buildHTML = function () {
        ...
        if (isCreative) {
            ...
            groups.push(
                '<span class="dk-kb-group">' +
                '<span class="dk-kb-keys">' + this._getKeyLabel(kb.TURBO_TOGGLE) + '</span>' +
                '<span class="dk-kb-label">turbo</span>' +
                '</span>'
            );
        }
