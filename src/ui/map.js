...
(function () {
    ...
    try {
      ...
      if (!this._fullMapCtx) {
        Donkeycraft.Logger.error(
          'MapPanelUI',
          'Failed to create 2D context for offscreen canvas'
        );
        return false;
      }
      return true;
  ...
  Donkeycraft.MapPanelUI.prototype._attachCanvasListeners = function (canvas) {
    ...
    canvas.addEventListener('wheel', this._handlers._onWheel, {
      passive: false,
      capture: true,
    });
   ...
   */
  Donkeycraft.MapPanelUI.prototype.setCanvases = function (
    fullMapCanvas,
    minimapCanvas
  ) {
  ...
  Donkeycraft.MapPanelUI.prototype._onWheel = function (e) {
    ...
    var worldXBefore =
      this._offsetX + (mouseX / canvasWidth) * (canvasWidth / this._zoom);
    var worldYBefore =
      this._offsetY + (mouseY / canvasHeight) * (canvasHeight / this._zoom);
    ...
    this._offsetX =
      worldXBefore - (mouseX / canvasWidth) * (canvasWidth / newZoom);
    this._offsetY =
      worldYBefore - (mouseY / canvasHeight) * (canvasHeight / newZoom);
    this._zoom = newZoom;
   ...
   * @private
   */
  Donkeycraft.MapPanelUI.prototype._centerOnPosition = function (
    worldX,
    worldZ
  ) {
   ...
   * @private
   */
  Donkeycraft.MapPanelUI.prototype._renderSurfaceBlocks = function (
    ctx,
    chunkX,
    chunkZ,
    canvasX,
    ...
    var chunk = this._chunkManager
      ? this._chunkManager.getChunkIfExists(chunkX, chunkZ)
      : null;
    if (!chunk) return;
  ...
  Donkeycraft.MapPanelUI.prototype._getBlockColor = function (blockId) {
    ...
    if (
      Donkeycraft.MapRenderer &&
      typeof Donkeycraft.MapRenderer._getBlockColor === 'function'
    ) {
   ...
   * @private
   */
  Donkeycraft.MapPanelUI.prototype._drawGridLines = function (
    ctx,
    worldLeft,
    worldTop,
    worldRight,
   ...
   * @private
   */
  Donkeycraft.MapPanelUI.prototype._renderPlayerIndicator = function (
    ctx,
    playerPos,
    yaw,
    w,
  ...
  Donkeycraft.MapPanelUI.prototype._renderOverlay = function (ctx, w, h) {
    ...
    if (
      this._chunkManager &&
      typeof this._chunkManager.getChunkCount === 'function'
    ) {
   ...
   */
  Donkeycraft.MapPanelUI.prototype.renderFullMap = function (
    playerPos,
    yaw,
    pitch
  ) {
  ...
  Donkeycraft.MapPanelUI.prototype.registerEscapeHandler = function () {
    ...
    if (
      Donkeycraft.Input &&
      typeof Donkeycraft.Input.registerKeyHandler === 'function'
    ) {
  ...
  Donkeycraft.MapPanelUI.prototype.destroy = function () {
    ...
    if (canvas) {
      if (this._handlers._onWheel)
        canvas.removeEventListener('wheel', this._handlers._onWheel);
      if (this._handlers._onMouseDown)
        canvas.removeEventListener('mousedown', this._handlers._onMouseDown);
      if (this._handlers._onMouseUp)
        canvas.removeEventListener('mouseup', this._handlers._onMouseUp);
      if (this._handlers._onMouseMove)
        canvas.removeEventListener('mousemove', this._handlers._onMouseMove);
      if (this._handlers._onMouseLeave)
        canvas.removeEventListener('mouseleave', this._handlers._onMouseLeave);
    }

    if (this._toggleBtn && this._handlers._onToggleClick) {
      this._toggleBtn.removeEventListener(
        'click',
        this._handlers._onToggleClick
      );
    }
    ...
    if (this._handlers._onKeydown) {
      if (
        Donkeycraft.Input &&
        typeof Donkeycraft.Input.unregisterKeyHandler === 'function'
      ) {
        Donkeycraft.Input.unregisterKeyHandler(
          'Escape',
          this._handlers._onKeydown
        );
      } else {
