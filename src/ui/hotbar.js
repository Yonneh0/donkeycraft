...
(function () {
  ...
  Donkeycraft.Hotbar.prototype._subscribeToStaminaEvents = function () {
    var self = this;
    this._onStaminaChanged = function (data) {
      self._onStaminaUpdate(data);
    };
    ...
    if (globalBus) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Hotbar',
          'Failed to subscribe to stamina:changed: ' + e.message
        );
      }
    } else {
      Donkeycraft.Logger.warn(
        'Hotbar',
        'No global EventBus instance available — stamina bar will not receive updates'
      );
    }
  };
  ...
  Donkeycraft.Hotbar.prototype._subscribeToManaEvents = function () {
    var self = this;
    this._onManaChanged = function (data) {
      self._onManaUpdate(data);
    };
    ...
    if (globalBus) {
      try {
        ...
        Donkeycraft.Logger.warn(
          'Hotbar',
          'Failed to subscribe to mana:changed: ' + e.message
        );
      }
    } else {
      Donkeycraft.Logger.warn(
        'Hotbar',
        'No global EventBus instance available — mana bar will not receive updates'
      );
    }
  };
  ...
  Donkeycraft.Hotbar.prototype._onManaUpdate = function (data) {
    ...
    if (delta > 0) {
      ...
      this._manaAnimationTimeout = setTimeout(
        function () {
          if (this._manaFill) {
  ...
  Donkeycraft.Hotbar.prototype._updateSelectionHighlight = function () {
    ...
    if (this._guiRenderer && this._canvas) {
      try {
        this._guiRenderer.renderHotbar(
          this._selectedSlot,
          this._canvas.width,
          this._canvas.height
        );
      ...
      if (oldStack !== stacks[i]) {
        if (this._listeners.onSlotChange) {
          for (var j = 0; j < this._listeners.onSlotChange.length; j++) {
            try {
              this._listeners.onSlotChange[j](i, stacks[i]);
            } catch (e) {}
          }
        }
      }
  ...
  Donkeycraft.Hotbar.prototype.setSelectedSlot = function (n) {
    ...
    if (this._listeners.onSelectedChange) {
      for (var i = 0; i < this._listeners.onSelectedChange.length; i++) {
        try {
          this._listeners.onSelectedChange[i](n, oldSlot);
        } catch (e) {}
      }
    }
  };
