...
(function () {
   ...
   */
  Donkeycraft.GuiManager.prototype.registerComponent = function (
    type,
    constructor
  ) {
  ...
  Donkeycraft.GuiManager.prototype.open = function (type, data) {
      ...
      try {
        ...
        if (Donkeycraft.Logger)
          Donkeycraft.Logger.error(
            'GuiManager: Failed to create UI for type "' +
              type +
              '": ' +
              e.message
          );
    ...
    try {
      var globalBus = Donkeycraft.EventBus.getGlobal
        ? Donkeycraft.EventBus.getGlobal()
        : null;
      if (globalBus) {
        globalBus.emit('gui:open', {
          type: type,
          stackDepth: this._guiStack.length,
        });
      }
    } catch (e) {}
    ...
    if (this._listeners._onOpen) {
      for (var i = 0; i < this._listeners._onOpen.length; i++) {
        try {
          this._listeners._onOpen[i](type);
        } catch (e) {}
      }
    }
  ...
  Donkeycraft.GuiManager.prototype.close = function () {
    ...
    if (this._listeners._onClose) {
      for (var i = 0; i < this._listeners._onClose.length; i++) {
        try {
          this._listeners._onClose[i](removed.type);
        } catch (e) {}
      }
    }
      ...
      try {
        ...
        if (Donkeycraft.Logger)
          Donkeycraft.Logger.warn(
            'GuiManager: UI handleKeyPress error: ' + e.message
          );
      }
    }
   ...
   */
  Donkeycraft.GuiManager.prototype.handleDrop = function (
    itemStack,
    slotIndex
  ) {
    ...
    if (top && top.ui && typeof top.ui.handleDrop === 'function') {
      try {
        top.ui.handleDrop(itemStack, slotIndex);
      } catch (e) {}
    }
  };
